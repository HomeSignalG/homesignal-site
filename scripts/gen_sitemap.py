...
def prerendered_pages():
    """The Box Elder pre-render test set (scripts/gen_zip_pages.py).

    ⚠️ CONTROLLED TEST — 18 ZIPs + 1 county hub, Box Elder County UT only. Nothing
    national depends on this; when the set is empty the sitemap is byte-identical to
    what it was before this function existed.

    A page is advertised ONLY if its own generated HTML says `index, follow`. Reading
    the artifact rather than re-deriving the rule is what makes it impossible for the
    sitemap to advertise a URL that carries a noindex — the exact contradiction the
    development ZIP pages shipped for 11,591 URLs (canonical/sitemap disagreement).

    Returns (zips_handled, urls) where zips_handled must be REMOVED from the legacy
    homesignalmap.html?zip= list so one ZIP is never advertised under two URLs.
    """
    try:
        import gen_zip_pages as gz          # same directory; sys.path[0] is scripts/
    except Exception as exc:                # never let the test set break the sitemap
        print(f"NOTE: pre-render module unavailable ({exc}); sitemap unchanged")
        return set(), []
    handled, urls = set(), []
    hub = os.path.join(gz.OUT_ROOT, "index.html")
    if os.path.exists(hub) and 'content="index, follow"' in open(hub, encoding="utf-8").read():
        urls.append(gz.HUB_URL)
    for z in gz.TEST_ZIPS:
        path = os.path.join(gz.OUT_ROOT, z, "index.html")
        if not os.path.exists(path):
            continue
        handled.add(z)                      # generated at all → owns this ZIP's dev URL
        if 'content="index, follow"' in open(path, encoding="utf-8").read():
            urls.append(gz.zip_url(z))
    return handled, urls
...
def main():
    ...
    # Pre-rendered ZIPs are served from their own static URL instead (Box Elder test set),
    # so they drop out of the legacy homesignalmap.html?zip= list — never both.
    prerendered_zips, prerendered_urls = prerendered_pages()
    dev_zips = [z for z in fetch_dev_zips()
                if z in set(comm_zips) and z not in prerendered_zips]
    lines = ['<?xml version="1.0" encoding="UTF-8"?>',
             ...
             f' + {len(prerendered_urls)} pre-rendered Box Elder page(s)'
    ...
    for loc in prerendered_urls:
        lines.append(url_el(loc, "daily", "0.8"))
    lines.append("</urlset>")
