...
- **Notices/Meetings tile split:** site half shipped (`box-elder.html` + `community.html` now write independent `users.topics.notices` / `users.topics.meetings`). Engine half (storage + `digest.py` two-section delivery + dry-run) pending in `homesignal-ingest` — full ready-to-apply spec in `docs/notices-meetings-split-engine-spec.md`. Fold into the unmerged email-delivery migration; no second migration.
