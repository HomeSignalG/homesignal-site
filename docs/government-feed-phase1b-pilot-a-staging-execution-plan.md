...
select count(*) as transition_count,
       max(transition_spec_version) as spec_version
from public.feed_candidate_transitions;
```

**Expected:** `transition_count = 41`, `spec_version` matches `transition-spec.v1.json` (`1.0`).
...
# Activation gates (fixture — offline)
node scripts/gov-feeds/activate-feed-candidate.mjs \
  --feed-id wake-county-nc-granicus-meetings \
  --candidate-json fixtures/gov-feeds/activation-gate-fixtures.json \
  --feed-json fixtures/gov-feeds/activation-gate-fixtures.json
```

**Note:** The activation-gate fixture file is an array; for a real preflight use exported JSON rows from staging (Operator Runbook § Activation).

**GitHub Actions smoke (optional):**

- `rollback-gov-feed-candidate` with defaults (`from_state=active`, `event=open_circuit`)
- `activate-gov-feed-candidate` with fixture paths once staging rows exported
...
|-------|-------------------|---------------|
| Discovery | `discovered` → `discriminated` → `validated` → `title_gate_verified` | Steps 1–3 |
| Registry | `title_gate_verified` (row insert) | New — `feed_candidates` INSERT |
| Insert | `inserted` | Steps 4–5 |
| Dry Run | `dry_running` → `dry_run_pass` | Steps 2–3 |
| Golive | `goliving` | Step 8 |
| Title Verification | `title_verified` | Step 9 |
| Activation | `activating` → `active` | Step 10 |
