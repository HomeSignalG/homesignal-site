...
| Global Best Practices / Emerging tiles empty? | **The subscription TAXONOMY is shared, one-time — the CONTENT is not automatic.** The universal topic *list* (News / Emerging / Global) is community-agnostic and needs **zero per-community config** (that is what "flows automatically" means, and it is solved). But every alert is still stored with a `community_id` — **no content is community-agnostic** (verified 2026-07: `select count(*) from alerts where community_id is null` = 0; only the 3 UT pilot communities have *any* alerts). So a new community's News/Emerging/Global tiles stay **empty until the ingest pipeline produces universal-tagged content for that community_id** — same as government content, just with no per-place *feed* to configure. "Automatic" = no config, **not** "content appears without the pipeline running for that community." |
...
| An **independent city shares ALL its ZIPs with the surrounding county** (the crosswalk lists the same ZIP under both fips — e.g. Fairfax city `22030/22031/22032` also under Fairfax County), so the `zipcodes` package attributes them to the county and the city root ends up **empty/unreachable**? | **Re-home those ZIPs to the independent city** (the more-specific jurisdiction — it has its own government), parent their ZIP pages to the city, and drop them from the county array (one county array per ZIP, §12.4). Post-build data audit catches this: `select … where level='county' and array_length(zip_codes,1) is null`. Functionally identical for residents (same canonical topics) and most-specific resolution is unchanged. Idempotent correction at the tail of the seed. Not a stop. |
