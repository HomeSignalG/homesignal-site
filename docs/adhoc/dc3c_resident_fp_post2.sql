...
select now()::text measured_at, (select md5(array_to_string(statements, E'\n')) from supabase_migrations.schema_migrations where version = '20260922184138') applied_stmt_md5, (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname like 'zz\_%') leftover_mutants, ndr.n ndr_n, ndr.fp ndr_fp, ap.n ap_dc_n, ap.fp ap_dc_fp, sp.n maps_n, sp.fp maps_fp, fn.n fn_n, fn.fp fn_fp,
