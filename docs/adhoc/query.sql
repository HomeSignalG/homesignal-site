...
with fn as (select pg_get_functiondef('public.app_refresh_zip(text)'::regprocedure) d),
...
select json_build_object(
 ...
 'water_gov_alerts', (select json_agg(t) from (select a.title, a.agency_name, c.name comm, a.created_at::date d,
     (select json_agg(distinct ac.category) from public.app_changes ac where ac.source_ref=a.source_url) mat_cats,
     (select count(distinct ac.zip) from public.app_changes ac where ac.source_ref=a.source_url) mat_zips
 ...
 'water_gov_alerts_materialized', (select json_build_object('alerts',count(*),'with_app_changes',count(*) filter (where exists(select 1 from public.app_changes ac where ac.source_ref=a.source_url))) from public.alerts a where a.pipeline_type='government_notice' and a.category='Water districts & utilities'),
 'negative_water_word_other_cat', (select json_agg(t) from (select a.category, a.title, (select json_agg(distinct ac.category) from public.app_changes ac where ac.source_ref=a.source_url) mat_cats
 ...
 'water_meetings', (select json_agg(t) from (select m.title, m.meeting_date::date, c.name comm,
     (select json_agg(distinct ac.category) from public.app_changes ac where ac.source_ref=m.source_url) mat_cats
 ...
 'local_news_env_example', (select json_agg(t) from (select a.title, a.subtopics, (select json_agg(distinct ac.category) from public.app_changes ac where ac.source_ref=a.source_url) mat_cats, (select count(distinct ac.zip) from public.app_changes ac where ac.source_ref=a.source_url) mat_zips
   from public.alerts a where a.category='local_news' and a.subtopics && array['Water Quality','Soil Quality','Air Quality'] and exists(select 1 from public.app_changes ac where ac.source_ref=a.source_url) order by a.created_at desc limit 3) t),
