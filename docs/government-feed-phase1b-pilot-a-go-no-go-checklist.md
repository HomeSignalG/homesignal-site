...
**Canonical execution order:** `docs/government-feed-phase1b-pilot-a-plan.md` §3

> Gate IDs (G0–G19) are **stable labels**; the tables below list them in
> **execution order**. Note G7 (dry run) follows G8–G9 because the implemented
...
| G7 | Dry run pass (post-insert) | `dryrun-gov-feed` or probe exit 0 | Operator | `dry_run_pass` state | **BLOCK** |
| G10 | Sync zero drift | `sync-feeds-config` exit 0 — **authoritative sync gate** | Operator | `has_drift: false` for Wake feed | **BLOCK** |
...
- [ ] **G7** — Dry run pass (post-insert); registry `dry_run_pass`

### Pre-activation

- [ ] **G10** — Sync check exit 0 (attach output) — authoritative sync gate; a hand-written `--sync-json` never substitutes for it
...
|----------------|--------|
| G4–G6 | Do not insert; fix discovery/title gate or abandon candidate |
| G8–G9 | Do not dry-run or golive; reconcile INSERT/CSV |
| G7, G10 | Do not golive; re-probe (`dry_run_failed` path) or resolve drift |
