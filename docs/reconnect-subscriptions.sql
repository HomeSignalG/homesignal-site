...
resolved as (
  select fo.user_id, fo.email, fo.zip, r.root_id, r.topics
  from follows fo
  cross join lateral (
...
preview as (
  select user_id, email, zip, root_id,
...
-- (a) create/refresh the digest identity for each (email, root) with a real floor
ins_users as (
  insert into public.users (email, zip_code, community_id)
  select distinct email, zip, root_id
  from preview
  where array_length(floor_topics, 1) > 0
  on conflict (email, community_id) do update set zip_code = excluded.zip_code
  returning 1
)
-- (b) add the floor subscriptions, matched back to the (new or existing) users row
insert into public.user_subscriptions (user_id, community_id, pipeline_type, topic)
