-- Repair migration: users geography invariant (forward-only).
-- Re-applies all geography functions, trigger, RPC writers, PostgREST grants,
-- and schema reload. Safe to run when the 20260721160000 migration was skipped
-- or only partially applied.

begin;

-- ---------------------------------------------------------------------------
-- 0) Clean up failed E2E account (must not leave Utah identity eligible).
-- ---------------------------------------------------------------------------
delete from public.user_subscriptions s
using public.users u
where s.user_id = u.id
  and lower(trim(u.email)) = 'texas-launch-8200c786d2@homesignal-e2e.test';

delete from public.users
where lower(trim(email)) = 'texas-launch-8200c786d2@homesignal-e2e.test';

-- ---------------------------------------------------------------------------
-- 1) community_covers_zip
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3) retire_stale_digest_identities — normalize email; make ALL other rows ineligible.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4) Geography invariant trigger (drop + recreate + enable).
-- ---------------------------------------------------------------------------
...
create trigger users_geography_invariant
  ...
  for each row
  execute function public.trg_users_geography_invariant();

alter table public.users enable trigger users_geography_invariant;
alter table public.users enable always trigger users_geography_invariant;

-- ---------------------------------------------------------------------------
-- 5) subscribe_area_defaults
-- ---------------------------------------------------------------------------
...
begin
  ...
  from public.users where lower(trim(email)) = v_email and community_id = v_community;
...
-- ---------------------------------------------------------------------------
-- 7) signup_complete — upsert + reconcile + retire stale (transactional).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 8) Grants for PostgREST RPC visibility.
-- ---------------------------------------------------------------------------
...
grant execute on function public.community_covers_zip(uuid, text) to service_role, authenticated, anon;
grant execute on function public.resolve_digest_community_id(text) to service_role, authenticated, anon;
grant execute on function public.retire_stale_digest_identities(text, uuid) to service_role;
...
-- ---------------------------------------------------------------------------
-- 10) Idempotent verification (safe read-only).
-- ---------------------------------------------------------------------------
select
  public.community_covers_zip('b0d7b834-4fcf-49bf-a018-0cb611de065c'::uuid, '78617') as covers_texas,
  public.resolve_digest_community_id('78617')::text as travis_id,
  exists (
    select 1 from pg_trigger t
    join pg_class c on c.oid = t.tgrelid
  join pg_namespace n on n.oid = c.relnamespace
    where n.nspname = 'public' and c.relname = 'users'
      and t.tgname = 'users_geography_invariant' and t.tgenabled in ('O', 'A')
  ) as trigger_enabled,
  position('retire_stale_digest_identities' in pg_get_functiondef(
    'public.signup_complete(text,uuid,text,jsonb,text,jsonb,boolean,text,text,text)'::regprocedure
  )) > 0 as signup_calls_retire;
