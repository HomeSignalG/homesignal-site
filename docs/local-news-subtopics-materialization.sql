-- Local-News topic filtering — page path (Phase B)
-- ============================================================================
-- Parked, APPLIED MANUALLY in the Supabase SQL editor (repo convention:
-- homesignal-site/CLAUDE.md §1 #3). Additive + idempotent; preserves every
-- existing row and all non-Local-News behavior.
--
-- WHY: the 12-topic multi-label classifier already writes alerts.subtopics text[]
-- (homesignal-ingest/classify_news_subtopics.py), and the email digest already
-- filters Local News by it — but the resident-facing ZIP page reads app_changes,
-- which had no subtopics column, so app_refresh_zip dropped the tags and the page
-- could not filter. This migration:
--   1. adds app_changes.subtopics text[]  (nullable; existing rows -> NULL);
--   2. CREATE OR REPLACE app_refresh_zip so the Local-News insert copies
--      alerts.subtopics -> app_changes.subtopics. This is the ONLY change to the
--      function; the two edited lines are the Local-News insert's column list
--      (+ subtopics) and its SELECT (+ a.subtopics). Every other insert (Planning
--      & zoning, Government & civic, Meetings, Government notices) is byte-identical
--      to the live definition (captured via pg_get_functiondef and transformed with
--      two unique-anchor replacements).
--
-- ROLLBACK: re-apply the PRIOR app_refresh_zip body (Local-News insert without the
-- subtopics column/select); optionally "alter table public.app_changes drop column
-- if exists subtopics". Dropping is not required — a NULL/extra column is inert for
-- every existing reader. The next app_refresh_all() run repopulates either way.
-- ============================================================================

alter table public.app_changes add column if not exists subtopics text[];
...
begin
  ...
  insert into public.app_changes (community_id, zip, category, title, plain_language, occurred_at, source_ref, confidence, lens, subtopics)
  select coalesce(_root,_cid), _zip, 'Local News',
    a.title,
    coalesce(nullif(a.description,''), 'Local news'||coalesce(' — '||nullif(a.agency_name,''),'')||'.'),
    coalesce(a.published_at::date, a.created_at::date), a.source_url, 'Medium', 'value', a.subtopics
  from public.alerts a
  where a.community_id = coalesce(_root,_cid)
