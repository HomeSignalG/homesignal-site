...
// The heading is the resident's whole-ZIP claim, and "All" is the half that distinguishes it
// from address mode's "projects near me" reading. Both halves are asserted (#1068).
ok(/^All development across ZIP 78617$/.test((z.within || '').trim()),
...
// "Showing development within <radius> of" — it must name the SUBJECT (development) and the
// DISTANCE, not just be a "Within …" fragment that reads the same in either mode (#1068).
ok(/^Showing development within .+ of$/.test((a.within || '').trim()),
  '8 address mode states the radius', a.within);
...
ok(/^All development across ZIP 80210$/.test((back.within || '').trim()),
...
ok(/^Showing development within .+ of$/.test((am.within || '').trim()),
  '12a address mode still states its radius', am.within);
