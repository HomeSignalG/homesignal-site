...
(function () {
  ...
  // app_projects.status is a CLOSED vocabulary, re-measured 2026-09-20 over record_kind
  // 'development' (3,000,229 rows): Operating 1,378,872 - Approved 1,264,479 -
  // Proposed 356,873 - Active 5. 'Decided' is a FIFTH member the materializer has always
  // been able to write and has NEVER written: measured the same day, 0 of 3,000,229 rows
  // carry it, because no connector had ever stamped a decision. It is mapped below so the
  // first one does not render as a grey unknown.
  ...
  HS.n5BucketFromStatus = function (status) {
    ...
    // 'Decided' is the materializer's word for a record whose source stated a decision
    // (public.app_refresh_zip). It BROWSES as a proposal — a denied application is a
    // historical proposal and must stay in the category a resident looks in — so it takes
    // the Proposed colour here, exactly as it does on Map 1. Falling through to 'unknown'
    // instead (the behaviour before 2026-09-20) turned it into a grey lifecycle-unknown
    // pin, which asserts we do not know its stage when in fact we know it precisely.
    // The decision itself is never carried by the colour: it is text, everywhere.
    if (s === 'decided') return 'proposed';
    return 'unknown';
  };
  ...
  HS.n5SiteFromRow = function (row, project, home) {
    ...
    // RESIDENTIAL DEVELOPMENT QUALIFICATION - the SAME gate as ZIP mode, applied at the one
    // place an address-mode site is made. Both modes must answer "is this residential
    // development" identically; only their GEOGRAPHY differs (whole-ZIP membership vs a real
    // geocoded HOME and a chosen radius). See lib/residential-qualify.js.
    if (HS.residentialGateDrops && HS.residentialGateDrops(site, p)) return null;
    return site;
  };
