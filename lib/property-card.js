...
(function () {
  ...
  card.ENFORCEMENT_FIELDS = ['source_agency', 'source_name', 'record_type', 'entity_name',
    ...
    // From the Government Source Archive (docs/government-source-archive.md): the agency's own URL
    // AND our preserved copy, because a government page disappearing is a fact about the website,
    // not about the action.
    'archive_storage_key'];

  /**
   * `gov_actions` statuses, split by what may be INFERRED from an absence and what requires an
   * affirmative government document. The card must never let the first read as the second: a
   * document vanishing from an agency site is not the action being vacated.
   */
  card.ACTION_STATUS = {
    inferable_from_absence: ['REMOVED_FROM_SOURCE', 'SOURCE_UNAVAILABLE'],
    requires_document: ['SUPERSEDED', 'VACATED', 'RESCINDED']
  };
  card.statusNeedsDocument = function (v) {
    return card.ACTION_STATUS.requires_document.indexOf(String(v || '').toUpperCase()) >= 0;
  };
  ...
  card.COPY = {
    ...
    module: {
      ...
      // A page vanishing is a fact about the WEBSITE. Saying so plainly is what stops it reading
      // as the action having been vacated — which would require a government document saying so.
      removedFromSource: 'This is no longer on the agency\u2019s website; our archived copy is still here.',
