...
(function () {
  ...
  function withDistPrefix(sentence, dist) {
    ...
    const prefix = 'About ' + dist + ' from this place, ';
  ...
  function fitLength(sentence, dist) {
    ...
    if (s.length < 80) {
      const pad = ' Review the official public record for specifics that apply near this place.';
  ...
  HS.projectImpact = function (p) {
    ...
    // ── A DECIDED APPLICATION GETS NO FORWARD-LOOKING SENTENCE (2026-09-20) ──────────
    // Every branch below is written in the future tense — "if approved…", "once
    // construction begins", "if this project moves ahead". Applied to an application a
    // body already REFUSED, each one describes a future that is not going to happen, on
    // the page of a resident who may be relying on it. `Decided` is what
    // public.app_refresh_zip writes for a record whose source stated a decision, and it
    // matched none of the branches, so it fell through to the generic "if this project
    // moves ahead" — the worst of them, because it sounds measured.
    //
    // The replacement asserts nothing about the outcome (`app_projects` carries no
    // outcome word today, and reading "denied" out of "decided" would be the fabrication
    // this workstream removes) and nothing about today (a decision on the record is not a
    // fresh check for an appeal or a re-filing). It points at the record instead.
    const decided = st === 'decided' || !!(p && p.decision);
    const dist = p.dist || '';

    let sentence = '';

    if (decided) {
      // Kept inside fitLength's 80–140 band so it is neither padded with a generic tail
      // nor truncated mid-clause — a truncated honesty note is not an honesty note.
      return fitLength(
        'This application was decided, so it is not a pending change near you.'
        + ' Open the official record for the decision.', dist);
    }
