...
(function () {
  ...
  // A few source categories name an ACTIVITY or a SECTOR, not a building type, and the
  // default head noun reads wrongly against them: "research development" parses as R&D
  // rather than a research building, and "civic public development" is not English.
  // These get a neutral head noun instead. Conservative on purpose — the mapping only
  // changes the HEAD NOUN and never the source's own word, so a research record becomes
  // a "research facility" and never a laboratory, and a civic record becomes a "civic
  // project" and never a school. Keyed on the real 78617 vocabulary (13 distinct values,
  // measured 2026-08-09); "industrial", "commercial" and "residential" are deliberately
  // absent because "industrial development" already reads correctly.
  const ACTIVITY_HEAD = {
    research:  { facility: 'research facility',   development: 'research facility' },
    civic:     { facility: 'public facility',     development: 'civic project' },
    public:    { facility: 'public facility',     development: 'civic project' },
    education: { facility: 'education facility',  development: 'education project' },
    medical:   { facility: 'medical facility',    development: 'medical facility' }
  };
  ...
  function nounFor(p) {
    const kind = isFacilityRecord(p) ? 'facility' : 'development';
    // A slashed type is two alternatives the source offered ("Civic/Public"), not a
    // compound. Take the FIRST — quoting one of the source's own words beats gluing
    // both into "civic public development".
    const raw = String((p && (p.type || p.use_type || p.layer || p.category)) || '')
      .split('/')[0].replace(/[-_]+/g, ' ').replace(/\s+/g, ' ').trim().toLowerCase();
    if (GENERIC_TYPE.test(raw)) return kind;
    for (let i = 0; i < SPECIFIC_NOUN.length; i++) {
      if (SPECIFIC_NOUN[i][0].test(raw)) return SPECIFIC_NOUN[i][1];
    }
    if (ACTIVITY_HEAD[raw]) return ACTIVITY_HEAD[raw][kind];
