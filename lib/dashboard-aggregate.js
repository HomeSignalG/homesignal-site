...
(function () {
  ...
  // ------------------------------------------------- What's Changing preview (8K) ----
  // THE DASHBOARD ALREADY TRUNCATED, AND SAID NOTHING. `deduped.slice(0, 12)` shipped with
  // no control and no disclosure; the only truncation notice on the page reports ZIPs
  // omitted past MAX_QUERY_ZIPS, never RECORDS omitted past the row cap. Measured against
  // production over all five residents who have places, no sampling: pools of 39 / 31 / 31
  // / 4 / 4, so THREE OF FIVE were losing 27, 19 and 19 records with nothing on the page
  // admitting it. Corpus-wide the median ZIP carries 13 eligible changes and 5,690 of
  // 11,163 ZIPs (51.0%) exceed 12 on a single ZIP alone. This function is the honesty fix;
  // the smaller bound is the briefing fix. They are separate goods and only the first is
  // a correctness claim.
  //
  // It is PURE: no DOM, no query, no clock, no mutation of the input. The page renders what
  // it returns and decides nothing — the same split Fix 8G established, and the reason this
  // is drivable offline in Node rather than only in a browser.
  //
  // 🔑 COMPLETENESS IS A SEPARATE QUESTION FROM COUNT, AND ONLY THE CALLER CAN ANSWER IT.
  // `items.length` is how many records we HOLD, which equals the true total only when no
  // limit was reached. Four independent limits can hide records, so `countKnown` is passed
  // IN rather than inferred here — a function that guessed it would be asserting
  // completeness it has no evidence for, which is the exact failure this label exists to
  // end.
  //
  // 🔑 BUT THE UNKNOWN BRANCH STILL CARRIES A NUMERAL, AND IT IS A DIFFERENT NUMERAL.
  // It used to read a bare "View more changes →" on the reasoning that any digit implies a
  // total we cannot support. That conflated two counts. `hiddenCount` is how many records
  // we HOLD and are not showing — arithmetic over the loaded collection, exact whether or
  // not a limit was reached — while `total` is a claim about the world. So the unknown
  // branch names the hidden count ("View 52 more changes →") and the known branch names the
  // total ("View all 60 changes →"). The word carries the claim: "more" is a promise about
  // this list, "all" a promise about the universe.
  //
  // Measured on the founder's own five places, which is what made the distinction concrete:
  // 40 civic changes (under the 120 cap, so that half IS complete) but 540 development
  // records against a per-ZIP fetch of 8 — Del Valle 512, Celina 24. `devCapped` is true and
  // will stay true, so this branch is the STEADY STATE for any resident with a busy ZIP,
  // not a rare degraded case. A permanently numberless control was the real cost.
  // ⚖️ THE BOUND IS 3 (founder, 2026-09-15). It was 8, and 8 was never a measured number —
  // Fix 8K picked it as "smaller than the silent 12". The briefing is a summary a resident
  // scans, not the list itself: three rows, then the control that names what is held back.
  // The HONESTY half of Fix 8K is untouched by the number — every record past the bound is
  // still disclosed and still one click away, and the label still distinguishes a hidden
  // count from a claimed total. Changing this constant is the whole change; the page reads
  // it through the view-model and decides nothing.
  var PREVIEW_LIMIT = 3;
  function expandUnknownLabel(hidden) { return 'View ' + hidden + ' more changes \u2192'; }
  var COLLAPSE_LABEL = 'Show fewer changes \u2191';

  function changesPreview(items, opts) {
    var list = items || [];
    opts = opts || {};
    var expanded = opts.expanded === true;
    // Absent evidence is NOT evidence of completeness: countKnown must be asserted true by
    // the caller, so a caller that forgets to compute it gets the conservative label.
    var countKnown = opts.countKnown === true;
    var total = list.length;
    var hasControl = total > PREVIEW_LIMIT;
    // slice() on both limbs — the expanded case returns a COPY, never the caller's array,
    // so a consumer that sorts or splices what it renders cannot reorder the canonical
    // collection underneath the next collapse.
    return {
      visible: (expanded && hasControl) ? list.slice() : list.slice(0, PREVIEW_LIMIT),
      total: total,
      hiddenCount: hasControl ? total - PREVIEW_LIMIT : 0,
      countKnown: countKnown,
      hasControl: hasControl,
      expanded: expanded && hasControl,
      expandLabel: countKnown
        ? ('View all ' + total + ' changes \u2192')
        : expandUnknownLabel(hasControl ? total - PREVIEW_LIMIT : 0),
  ...
  function followingCards(opts) {
    ...
    var cards = ids.slice(0, cap).map(function (id) {
      var p = byId[id];
      if (!p) return { id: id, title: 'Followed development', context: null, place: null, zip: null, unresolved: true };
      var status = p.status == null ? '' : String(p.status).trim();
      var type = p.type == null ? '' : String(p.type).trim();
      var zip = p.zip != null && /^\d{5}$/.test(String(p.zip)) ? String(p.zip) : null;
      return {
        id: id,
        title: (p.name == null ? '' : String(p.name).trim()) || 'Followed development',
  ...
  // ------------------------------------------------ unified My Places rail ----
  // ONE CARD, THREE TYPED GROUPS — and the split happens HERE, never in membership.
  // `canonicalPlaces` and `queryZips` are untouched by everything below: a Development
  // is not a Place, does not enter the monitored-place count, and contributes no ZIP to
  // any content read. This is a VIEW over collections the page already holds.
  //
  // THE COUNT IS THE COMPLETE COLLECTION, NOT THE VISIBLE SLICE. `count` is taken before
  // the cap, so an overflow line can never contradict the heading beside it, and a group
  // whose rows were capped still tells the truth about how many exist.
  var RAIL_PREVIEW_LIMIT = 3;

  // Resident-facing labels, word-for-word the same as My Places' OWN segmented control
  // (properties.html ships buttons reading "Addresses" / "ZIP Codes" / "Developments"), so the
  // two surfaces name a thing identically and the overflow link lands on a tab carrying the
  // label it promised. The internal kind keys (`address`/`zip`) and the route values
  // (`addresses`/`zips`/`projects`) are deliberately NOT renamed to match them — the route
  // value is a whitelisted literal in properties.html and a rename would break every
  // existing link and follow. Display and storage are separate vocabularies on purpose.
  //
  // THE ICONS COME FROM THE GLYPH VOCABULARY THE NAV ALREADY SHIPS, never a new icon set:
  // partials/shell.html uses `⌂` for Address/My Places and `◈` for Development, so the nav
  // and the rail mark the same thing the same way. `⌖` is the one addition — there is no
  // existing ZIP glyph to reuse. They are DECORATIVE: the heading already says the word, so
  // they render aria-hidden and a reader that drops them loses nothing.
  var RAIL_PLACE_GROUPS = [
    { kind: 'address', label: 'Addresses', icon: '\u2302', view: 'addresses' },
    { kind: 'zip', label: 'ZIP Codes', icon: '\u2316', view: 'zips' }
  ];
  var RAIL_DEV_LABEL = 'Developments';
  var RAIL_DEV_ICON = '\u25C8';
  var RAIL_DEV_VIEW = 'projects';   // resident-facing "Developments", internal route "projects"

  function railOverflowLabel(count, label) {
    return 'View all ' + count + ' ' + label + ' →';
  }
  function railHref(view) {
    return 'properties.html?view=' + view;
  }

  // A group descriptor the page renders without deciding anything. `overflow` is the number
  // NOT shown, so a group of exactly the limit reports 0 and the page renders no control —
  // an overflow link that promises nothing is the defect this guards against.
  function railGroup(opts) {
    opts = opts || {};
    var all = opts.items || [];
    var limit = opts.limit == null ? RAIL_PREVIEW_LIMIT : opts.limit;
    var count = opts.count == null ? all.length : opts.count;
    var visible = all.slice(0, limit);
    var hidden = Math.max(0, count - visible.length);
    return {
      kind: opts.kind,
      label: opts.label,
      icon: opts.icon || '',
      ...
      overflowLabel: railOverflowLabel(count, opts.label),
      href: railHref(opts.view)
    };
  }

  // The two PLACE groups, split out of the one canonical membership array. Order is fixed
  // (Addresses then ZIP Codes) and matches My Places' own segmented control.
  function placeGroups(places, limit) {
    return RAIL_PLACE_GROUPS.map(function (g) {
      return railGroup({
        kind: g.kind,
        label: g.label,
        icon: g.icon,
        view: g.view,
        limit: limit,
        items: (places || []).filter(function (p) { return p && p.kind === g.kind; })
      });
    });
  }

  // The DEVELOPMENTS group, built from followingCards' own id-derived total. The count must
  // come from `following.total` rather than the rendered rows: projectsByIds swallows a
  // rejection into an empty array, so counting rows would report a failed read as zero
  // follows — the false-empty this whole module exists to prevent.
  function developmentsGroup(following, limit) {
    following = following || { cards: [], total: 0 };
    return railGroup({
      kind: 'development',
      label: RAIL_DEV_LABEL,
      icon: RAIL_DEV_ICON,
      view: RAIL_DEV_VIEW,
      limit: limit,
      count: following.total || 0,
      items: following.cards || []
    });
  }

  // ------------------------------------------- Development Updates · Premium ----
  // A ROADMAP STATEMENT, exactly like PREMIUM_QOL above, and constant for the same reason:
  // it reports on nothing, so it reads identically whether every source succeeded or every
  // source failed. The copy is FUTURE TENSE throughout and names no capability that exists
  // today — there is no join from a followed development to an official-record stream
  // (app_follows.target_id is a soft reference to app_projects; resolved_project_status is
  // keyed to a different table entirely), so any present-tense claim here would be a
  // statement the data cannot support.
  var PREMIUM_DEV_UPDATES = {
    sectionLabel: 'DEVELOPMENT UPDATES · PREMIUM',
    availability: 'Coming soon',
    body: 'We’re building a way to follow one development’s own official record over '
      + 'time—the filings, hearings, and decisions that belong to it.',
    today: 'Today, saving a Development to My Places lets you reopen it. It does not send alerts.',
    cta: 'Get Premium access →'
  };
  // The acquisition attribution for this card. Matches the shape the other feature-level
  // CTAs already use ('ZIP Community Profile', 'Property Insights', 'Dashboard
  // Quality-of-Life') and rides the canonical waitlist path unchanged.
  var PREMIUM_DEV_UPDATES_SOURCE = 'Development Updates';
  // A SOURCE AND NOTHING ELSE — no zip key, no address key, no development id. This card is
  // feature-level, not place-level: there is no field for a place to arrive in, so a lead
  // can never be attributed to one arbitrary ZIP or one saved development.
  function devUpdatesLeadContext() { return { source: PREMIUM_DEV_UPDATES_SOURCE }; }
  ...
  var api = {
    ...
    railGroup: railGroup,
    railOverflowLabel: railOverflowLabel,
    RAIL_PREVIEW_LIMIT: RAIL_PREVIEW_LIMIT,
    RAIL_DEV_LABEL: RAIL_DEV_LABEL,
    RAIL_DEV_ICON: RAIL_DEV_ICON,
    RAIL_PLACE_GROUPS: RAIL_PLACE_GROUPS,
    PREMIUM_DEV_UPDATES: PREMIUM_DEV_UPDATES,
    PREMIUM_DEV_UPDATES_SOURCE: PREMIUM_DEV_UPDATES_SOURCE,
    devUpdatesLeadContext: devUpdatesLeadContext,
    ...
    CHANGES_QUERY_LIMIT: 120,
    DEV_QUERY_LIMIT: 8,
    expandUnknownLabel: expandUnknownLabel,
