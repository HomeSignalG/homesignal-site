...
(function () {
   ...
   *
   * EVERY DATA CENTER THEME POST, PROJECT-BACKED OR NOT. Membership is HS.mapsSocialThemeKey
   * (Map 1's own classifier). No keyword matching is introduced here — grep this file for
   * "data cent" and you will find it only in prose, exactly as lib/maps-social-theme.js
   * promises.
   *
   * ⚖️ THE ABSENCE CARVE-OUT IS GONE — FOUNDER RULING, 2026-09-21: every MAPS post gets a
   * map, including the "no data center filings" one. It used to read
   * `if (HS.mapsSocialIsAbsence(post)) return false;` on the reasoning that "there is no
   * marker to isolate and no map state to photograph". The first clause is true and the
   * second does not follow: THE MAP STATE IS THE WHOLE POINT of an absence picture. Its
   * subject is a ZIP's Map 1 with all four STATUS controls on, Data center the only PROJECT
   * TYPE and REGULATORY off, showing nothing — which is only a meaningful photograph
   * BECAUSE the policy was applied and measured. An unpoliced absence capture could be a
   * screenshot of a map with the type chip unchecked, and nobody could tell the difference
   * from the image.
   *
   * 🔑 EVERY LIMB OF THE POLICY STILL RUNS ON AN ABSENCE CAPTURE — only the "is the target
   * still drawn" check is skipped, and only because there is no target. That guard was
   * already written for it (`if (targetKey && !rendered.target_on_map)`, line ~463), which
   * is how this ruling needed no change to the verifier itself.
   */
  HS.mapsDcCapturePolicyApplies = function (post) {
    ...
    // ⚠️ `&& !!e.project_id` WAS REMOVED HERE, and it was the absence carve-out wearing a
    // second hat rather than a check about stamps. Keeping it would have left an absence
    // draft ungoverned whenever the classifier module had not loaded — i.e. exactly the
    // fail-closed path where the stamp is the only evidence there is.
    var stamped = (e.theme === DC_THEME);
  ...
  HS.mapsDcCapturePolicyEvidence = function (visual) {
    ...
    if (!r || typeof r !== 'object') {
      ...
      // ⚖️ A CAPTURE THAT SOUGHT NO TARGET IS NOT A CAPTURE THAT LOST ONE — founder ruling,
      // 2026-09-21. An absence post's picture is a ZIP's Map 1 under this exact policy with
      // NOTHING drawn, so demanding a drawn target of it would refuse every absence capture
      // forever, which is how "every post gets a map" would have silently not happened.
      //
      // ⚠️ THE DEFAULT IS "A TARGET WAS SOUGHT", DELIBERATELY. Every record stored before
      // `target_sought` existed carries no such key, and all of them are project captures —
      // so `!== false` keeps this assertion firing on all of them rather than quietly
      // excusing them. The permissive reading would have turned a real guard off for the
      // entire existing corpus at the moment it was widened.
      if (r.target_sought !== false && r.target_on_map !== true) {
        problems.push('the target project was not measured as drawn on the map');
      }
      // ⛔ DO NOT ADD `target_sought === false ⇒ on_map_total must be 0`. It was written
      // here, and its own numbers killed it: the absence post's sentence is WINDOW-BOUNDED
      // ("No data center filings **from the last N months** appear in …"), while this policy
      // puts all four STATUS controls ON, so the map legitimately draws data centres that
      // were filed outside that window — an OPERATING one from years ago being the obvious
      // case. Asserting an empty map would refuse the capture for a post whose sentence is
      // true, and it would do so by silently substituting "no records ever" for "no recent
      // filings". The count is RECORDED in `on_map_total` either way, so a reader can see
      // what the picture showed; it is simply not a contradiction.
  ...
  function readRendered(targetKey) {
    ...
    var out = {
      // ⚖️ `target_sought` EXISTS BECAUSE AN ABSENCE CAPTURE HAS NO TARGET — founder ruling,
      // 2026-09-21: every MAPS post gets a map, including the "no data center filings" one.
      // Without it a stored absence record is indistinguishable from a project capture whose
      // marker vanished, since both read `target_on_map: false`. The two are opposite facts:
      // one is the picture working exactly as intended, the other is a capture that must be
      // refused. The verifier already had this distinction (`if (targetKey && …)`); this
      // carries it into the STORED record so the offline validator can make it too.
      target_sought: !!targetKey,
  ...
  HS.mapsDcCapturePolicyRecord = function (applyResult, verifyResult) {
    return {
      ...
      // evidence that a particular project was shown — and so the enforced SQL guard
      // (public.hs_maps_dc_capture_policy_violations) can waive `target_on_map` for the
      // right reason rather than inferring it from the row's shape.
      ...
      rendered: {
        // Carried through so the offline validator can tell "no target was sought" from
        // "the target was not drawn". See readRendered().
        target_sought: verifyResult.rendered.target_sought,
