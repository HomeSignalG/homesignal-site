...
(function () {
  ...
  HS.mapsCaptureState = function (post) {
    ...
    // ⚖️ SUPERSEDED BY A LATER FOUNDER RULING — EVERY POST GETS A MAP, even when no
    // project can be pinned in that ZIP. The carve-out that stood here returned INELIGIBLE
    // for an absence post on the premise that "a post with no project has nothing on the
    // map to photograph". That premise is now false: the capture job photographs the ZIP
    // itself at `scope: "zip"`, so an absence post IS photographable and a capture IS owed.
    //
    // ⚠️ THE DEFECT #1273 FIXED IS NOT BEING REOPENED — read this before "restoring" it.
    // Its complaint was that identical absence rows gave OPPOSITE answers depending on
    // whether an out-of-band worker had happened to stamp `evidence.visual`: sixteen read
    // AWAITING and two read CAPTURE_INELIGIBLE. Under this ruling AWAITING is the CORRECT
    // reading for an unphotographed absence post, because the capture really is coming, and
    // the stamped rows converge on it as the job revisits them. The answer stops depending
    // on the stamp because the capture stops being optional — which is the same cure by a
    // different route.
