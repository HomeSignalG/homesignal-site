...
(function () {
  ...
  // ── EVERY MAPS CAPTURE IS THE MAP 1 PRODUCT CARD — FOUNDER REQUIREMENT, 2026-09-22 ──
  // The founder saw an ordinary MAPS draft on the Acquisition Dashboard with a bare map and
  // no STATUS / PROJECT TYPE / REGULATORY panel beside it, while the Data Center Theme
  // drafts carried that panel, and asked for the Data Center framing on every MAPS post.
  // Until then an ordinary capture clipped to `#map` and a Data Center capture clipped to
  // `.card.mapcard` in embed mode (scripts/maps-social-image.mjs).
  //
  // The FRAME is part of what a picture is of, so it rides in the key: an ordinary image
  // taken under the old bare-map frame must stop counting as this draft's image and be
  // re-taken, which is exactly what a key change does (and the object path, derived from
  // the key, moves with it rather than overwriting the old PNG).
  //
  // ⛔ IT IS APPENDED FOR THE POSTS THE DATA CENTER POLICY DOES NOT GOVERN, AND ONLY THOSE.
  // A Data Center key already carries the policy segment, and a policy capture has always
  // been the card, so those keys come out BYTE-FOR-BYTE unchanged and none of their images
  // is invalidated. Never a bumped 'v1' prefix: public.hs_maps_map_gate_violations matches
  // `v1|<zip>|<project>|%` and would refuse every row.
  var MAP1_CARD_FRAME = 'frame=map1-card@1';

  function pushFrameSegment(post, parts) {
    // FAILS CLOSED ON A MISSING POLICY MODULE the same way mapsCaptureBound does: with no
    // way to tell a Data Center post from an ordinary one, no segment is claimed at all,
    // and mapsCaptureBound then refuses to call anything bound.
    if (typeof HS.mapsDcCapturePolicyApplies !== 'function' || !HS.MAPS_DC_CAPTURE_POLICY) return;
    parts.push(HS.mapsDcCapturePolicyApplies(post)
      ? HS.MAPS_DC_CAPTURE_POLICY.key
      : MAP1_CARD_FRAME);
  }
   ...
   */
  /**
   * True when the stored visual names one scope and its capture_policy names another.
   * Either field alone, or both equal, is not a conflict. See `mapsCaptureBound`.
   */
  HS.mapsCaptureScopeConflict = function (visual) {
    var v = visual || {};
    var a = v.scope, b = (v.capture_policy || {}).scope;
    return !!a && !!b && a !== b;
  };
  ...
  HS.mapsCaptureKey = function (post, scope) {
    ...
    //
    // 🛑 SUPERSEDED 2026-09-22 FOR ORDINARY POSTS ONLY: their picture is NO LONGER "exactly as
    // valid as it was" — the founder required the Map 1 card (with its STATUS / PROJECT TYPE
    // / REGULATORY panel) on every MAPS post, so an ordinary key now carries MAP1_CARD_FRAME
    // and its bare-map images are re-taken. The prefix is still never bumped, and Data
    // Center keys are still byte-for-byte unchanged. See MAP1_CARD_FRAME above.
    pushFrameSegment(post, parts);
    return parts.join('|');
  };
  ...
  HS.mapsCaptureBound = function (post) {
    ...
    //
    // ⛔ A RECORD THAT CONTRADICTS ITSELF ABOUT ITS SCOPE IS NOT BOUND. When `visual.scope`
    // and `visual.capture_policy.scope` both state a scope and disagree, this module read the
    // first while public.hs_maps_dc_capture_policy_violations reads the second — so the
    // dashboard unlocked Approve on a row the database then refused at the trigger (64165,
    // 2026-09-22: visual 'zip', capture_policy 'project', the capture builder's default).
    // Refusing is the conservative side of that disagreement, and it makes the row due for
    // a fresh capture, which now records one scope in both places.
    if (HS.mapsCaptureScopeConflict(v)) return false;
  ...
  HS.mapsMapGateBlock = function (post) {
    ...
    if (!post.image_bucket_path) {
      ...
      why = 'The stored image records no capture key, so nothing says which draft it is a '
        + 'picture of.';
    } else if (HS.mapsCaptureScopeConflict(v)) {
      why = 'The stored image records two different scopes (' + v.scope + ' on the picture, '
        + v.capture_policy.scope + ' in its map-state record), so it cannot say what it is a '
        + 'picture of.';
  ...
  HS.MAPS_CAPTURE_FRAME = MAP1_CARD_FRAME;
}());
