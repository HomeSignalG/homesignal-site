...
HS.onReady(async function () {
  ...
  // It is an AREA label (not `precise`), so on a saved address's own ZIP the control still
  // names that address as Viewing (see shell.js::paintTopbar). Geography gate unchanged.
  if (meta && meta.name && HS.setViewLabel) HS.setViewLabel(meta.name + ' · ' + zip);
