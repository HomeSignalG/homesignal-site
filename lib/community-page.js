...
HS.onReady(async function () {
  ...
  // ── PCM-4 · THE ZIP CONTEXT MAP — authenticated only ──────────────────────────────────
  //
  // The geography IS the polygon. This ZIP's real ZCTA outline, fitted to its own bounds —
  // not a centroid, not a radius, and NOT a bounding box of the project points on the page.
  // That last one is the shape #1141 shipped and had closed: a bbox of records is a picture
  // of what we happen to hold, presented as the area a resident is following.
  //
  // Nothing is plotted on it. Pinning every project in the ZIP here would make a second
  // Map 1, and Map 1 already exists one click away through the "View Development Map →"
  // control in the header above — which is why this block deliberately adds NO second
  // Development link of its own.
  //
  // SAME A-022 GATE AS ZIP HEALTH. An anonymous or ?demo=1 visitor concatenates '' and the
  // public PS-001 output is byte-for-byte what it was: the Follow control, the score strip,
  // the pulse, FM-078. This sits beside them and replaces none of them.
  //
  // The gate is also what keeps the RPC honest: public.app_zcta_boundary has NO EXECUTE for
  // anon (proved live in PCM-3), so calling it signed-out would be a guaranteed console
  // error on the protected public document. It is never called outside this branch.
  var zipContextMap = (sess && !sess.demo)
    ? '<div class="block" id="zipContext">'
      + '<h2 class="bt">The area you\'re following</h2>'
      + '<div class="mapwrap" id="zipMap" style="height:clamp(200px,42vw,320px)"></div>'
      + '<div class="quiet" id="zipMapNote" style="margin-top:10px">Loading this ZIP\'s boundary…</div>'
      + '</div>'
    : '';

  // Painted after the DOM exists. Reads the polygon through the SECURITY DEFINER function —
  // never geo.zcta_boundary directly, which the browser roles cannot see and must not.
  async function paintZipBoundary(z) {
    var el = document.getElementById('zipMap');
    if (!el) return;
    var note = document.getElementById('zipMapNote');
    var payload = null;
    try {
      var r = await HS.sb().rpc('app_zcta_boundary', { p_zip: z });
      if (!r.error) payload = r.data;
    } catch (e) { payload = null; }

    var geom = payload && payload.geometry;
    var drawable = !!payload && payload.status === 'boundary_complete' && !!geom
      && (geom.type === 'Polygon' || geom.type === 'MultiPolygon');

    // NO POLYGON MEANS NO MAP. HS.buildLive would refuse a fitBoundary with nothing to fit,
    // and refusing is correct — but the refusal is the LIBRARY's, and the resident is owed a
    // sentence either way. `not_measured` and "the read failed" are different facts and are
    // told apart here, the same way lib/zip-authoritative.js keeps null and [] apart.
    // Nothing is drawn in their place: no circle, no envelope, no centroid.
    if (!drawable) {
      el.innerHTML = '';
      el.setAttribute('data-hs-map-refused',
        payload && payload.status === 'not_measured' ? 'not-measured' : 'unavailable');
      if (note) {
        note.textContent = (payload && payload.status === 'not_measured')
          ? 'We don\'t have a mapped boundary for ZIP ' + z + ' yet. We won\'t draw an '
            + 'approximation of it.'
          : 'The boundary for ZIP ' + z + ' couldn\'t be read just now.';
      }
      return;
    }

    // THE CAPTION IS SET BY THE ENGINE, NOT BY THE CALL. Setting the success sentence
    // straight after buildLive() claims a drawn boundary the moment the call returns —
    // but the tile engines resolve asynchronously, and when neither loads, buildLive
    // refuses with `boundary-needs-tiles` (PCM-2's contract: schematic must never fake a
    // polygon with a circle). Measured in the sandbox, where jsDelivr is unreachable: the
    // box was empty and the page still read "The whole of ZIP 84302, from the U.S. Census
    // ZCTA boundary." A caption is a claim; it may only be made by the path that actually
    // drew it. onReady fires on Leaflet and MapLibre ONLY — never schematic — which is
    // exactly the discrimination this needs, so the two handlers are mutually exclusive by
    // construction rather than by a flag we maintain.
    HS.buildLive(el, {
      boundary: geom,          // the publisher's own outline, already WGS84 from the reader
      fitBoundary: true,       // the polygon's bounds drive the viewport — not center+zoom
      items: [],               // the geography is the subject; this is not a project map
      interactive: true,
      onReady: function () {
        if (note) note.textContent = 'The whole of ZIP ' + z + ', from the U.S. Census ZCTA boundary.';
      },
      onRefuse: function () {
        // The boundary IS on file — the reader just returned it. Only the map failed to
        // draw, and saying so is different from `not_measured` above.
        if (note) note.textContent = 'We have the boundary for ZIP ' + z
          + ', but the map couldn\'t load just now.';
      }
    });
  }
  ...
  if (zipContextMap) paintZipBoundary(zip);
