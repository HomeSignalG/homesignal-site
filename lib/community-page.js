...
HS.onReady(async function () {
  ...
  // A ZIP Code Place is geography, never HOME: no house pin, no radius, frame the
  // records that belong to this ZIP. Address Places (property.html) are the only
  // surface that may pass a home. "View Development Map →" stays the Map 1 CTA.
  function placeMapBlock(mapHref) {
    return '<div class="block" id="placeMapBlock" style="padding:16px 18px;margin-top:14px">'
      + '<div class="bt-row"><h2>Map 1</h2><a href="' + HS.esc(mapHref)
      + '" data-znav="homesignalmap.html" id="placeMapLink" style="font-size:12.5px;color:var(--green);text-decoration:none;font-weight:600">Open full map →</a></div>'
      + '<div class="mapwrap" id="placeMap" role="region" aria-label="Map of this ZIP Code — open the full map"></div>'
      + '</div>';
  }
  function mountZipMap(items, center) {
    var el = document.getElementById('placeMap');
    if (!el || !HS.placeMap) return;
    HS.placeMap(el, {
      mode: 'zip',
      home: null,
      center: center,
      items: items || [],
      mapHref: HS.navHref('homesignalmap.html', zip),
  ...
  function zipMapCenter(m) {
    return (m && m.lat != null && m.lng != null) ? { lat: m.lat, lng: m.lng } : null;
  }
  ...
  if (status !== 'pass') {
    ...
    page.innerHTML = '<div class="ph"><div class="eyebrow">Communities</div><h1>' + HS.esc(zip) + (meta && meta.name ? ' · ' + HS.esc(meta.name) : '') + (meta && meta.state ? ', ' + HS.esc(meta.state) : '') + '</h1>'
      ...
      + '</div></div>'
      + placeMapBlock(HS.navHref('homesignalmap.html', zip))
  ...
  mountZipMap(projects.concat(facilities), zipMapCenter(c));
