...
HS.onReady(async function () {
  ...
  // `complete === false` with no `unavailable` stamp is a FAILED read (network, or a
  // statement timeout under load). It is folded in here rather than left to fall through,
  // because falling through renders the ordinary empty section — which tells a resident
  // there are no records when in truth we could not look.
  var devUnavailable = (projects && projects.unavailable) ? String(projects.unavailable)
                     : (projects && projects.complete === false) ? 'read_failed'
                     : null;
  ...
  page.innerHTML =
    ...
    + HS.tpl.statTile(HS.zipDevelopmentCount(projects, devTotal), 'Development projects', '')
