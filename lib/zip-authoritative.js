...
(function () {
  ...
  // (founder rule, restated 2026-09-22 for EVERY Map 1 Type and EVERY source.) Every ZIP-mode
  // site is assembled HERE (HS.zipModeSites) and nowhere else, so a plane cannot be appended
  // beside it. This module READS membership evidence produced by the server; it never
  // re-decides it, and it reads no coordinate, distance, Type or source field to do so.
  //
  //   1. AUTHORITATIVE DEVELOPMENT — built by zipAuthSiteFromMarker from a COMPLETE
  //      app_zip_projects_markers payload (N5: exact ST_Intersects against the ZCTA polygon).
  //   2. NATIONAL DATA-CENTRE PLANE — each row carries `zip_membership`, the verdict of the
  //      canonical predicate geo.zip_point_membership_in (public.national_dc_zip_members,
  //      docs/zip-membership-canonical.sql). Only 'member' is admitted; anything else — no
  //      verdict, 'outside', 'not_measured', an unknown value, a future source that never went
  //      through the database — is not.
  //   3. THE FACILITY PLANE (EPA FRS points cached in development_reports.sites) — read ONLY
  //      through public.zip_mode_report_sites, which returns each facility with the same
  //      canonical verdict and serves members only. The page no longer downloads the raw sites
  //      array at all. (The first design rewrote those arrays in the database and coincided with
  //      three production restarts; this one writes nothing — CLAUDE.md §7.08.)
  //   Every non-development POINT therefore needs `zip_membership === 'member'` here, whatever
  //   plane or source it came from. Do NOT "fix" a missing verdict by testing coordinates in
  //   this module — a record without the server's verdict is simply not shown.
  HS.ZIP_MEMBER = 'member';
  HS.zipMemberAdmitted = function (rec) {
    return !!rec && rec.zip_membership === HS.ZIP_MEMBER;
  };

  // ZIP-MODE MERGE of the report's sites with authoritative development. The report's own
  // DEVELOPMENT points are candidates (the engine retrieved them around the ZIP centroid) and
  // are REPLACED by authoritative whole-ZIP development. Of everything else:
  //   * facility-plane points (scope point, not development) pass ONLY on a 'member' verdict
  //     (public.zip_mode_report_sites stamps it; a raw cached point carries none and is dropped);
  //   * area / jurisdiction notices (scope area) keep their county/city-wide treatment — they
  //     are not point claims.
  HS.zipAuthMergeSites = function (reportSites, authSites) {
    ...
    // Authoritative development carries its evidence flag, set only by zipAuthSiteFromMarker.
    return kept.concat((authSites || []).filter(function (s) { return s && s.zip_authoritative === true; }));
  };

  // THE WHOLE ZIP-MODE POPULATION, assembled in one place. `nationalSites` are the national
  // plane's records already mapped to sites; each must carry the server's `zip_membership`,
  // and the whole plane additionally requires this ZIP's geography to be complete (Fix 29 —
  // one definition, delegated). A future plane is added HERE, with its evidence, or it does
  // not reach the map.
  ...
  // THE FACILITY READ'S OUTCOME, and the count that goes with it. `rs` is the body of
  // public.zip_mode_report_sites. The count is the MEMBER count — the same population the map
  // draws — and it is null (unknown, shown as a dash) whenever the facility plane is not a
  // measurement: no boundary, a failed read, or a shape we do not recognise. Never the engine's
  // counts.facilities, which counts the radius-derived candidates.
  HS.zipReportOutcome = function (rs) {
    if (!rs || typeof rs !== 'object' || !isArr(rs.sites)) return 'unavailable';
    if (rs.status === 'complete' || rs.status === 'not_measured' || rs.status === 'no_report') return rs.status;
    return 'unavailable';
  };
  HS.zipReportSites = function (rs) {
    return HS.zipReportOutcome(rs) === 'unavailable' ? [] : rs.sites;
  };
  HS.zipFacilityMemberCount = function (rs) {
    const o = HS.zipReportOutcome(rs);
    if (o === 'no_report') return 0;
    if (o !== 'complete') return null;
    const n = rs.facility_counts && rs.facility_counts.member;
