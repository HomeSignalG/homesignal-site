...
(function () {
  ...
  // The national data-center plane (public.national_dc_for_zip) selects records within a
  ...
  //
  // WHAT THIS IS NOT. It does not test whether a record sits INSIDE a polygon. A ZIP that HAS
  // geography keeps exactly today's behaviour, including any national record near the centroid
  // but outside the boundary — that population belongs to Fix 28 and is untouched here.
  ...
  HS.zipAuthNote = function (payload, zip, sites) {
    ...
    if (outcome === 'not_measured') {
      return 'Development coverage for ' + z + ' is not measured yet — we will not estimate it '
           + 'from a circle around the ZIP centre. Enter an address for the live view '
           + 'around that address.';
    }
