...
(function () {
  ...
  // ── PROJECT GRAIN vs GEOMETRY-MARKER GRAIN ────────────────────────────────
  // Rule 4 above says the grain of `sites` is the MARKER: one project can own several
  // authoritative markers, and it must, because that is how a road corridor's geography is
  // drawn (marker_rule LINE_MERGED_COMPONENT_INTERVAL_1000M places one marker per ~1km of
  // the corridor CLIPPED TO THIS ZCTA). Those markers are geographically truthful and stay
  // on the map.
  //
  // What must NOT ride at marker grain is anything that states HOW MANY PROJECTS there are:
  // the headline counter and the Proposed/Approved/Operating rails. Measured 2026-09-05 on
  // ZIP 84029: 8 real projects, 221 markers, of which ONE UDOT project ("I-80/84; WWD RAMP
  // UPGRADES") owns 188 - so the page drew 221 pins, listed 221 rows, and counted 221, while
  // its own freshness line (zipAuthProjectCount, below) correctly said 8. Nationally 4,813 of
  // 7,663 populated ZIP pages overstated, and road-corridor line geometry accounted for
  // 91,046 of the 99,402 excess markers.
  //
  // This collapses a marker set to ONE representative site per project, for the COUNT and LIST
  // surfaces only. Never pass its output to the map.
  //
  // THREE THINGS THAT ARE EASY TO GET WRONG HERE
  //
  // 1. A SITE WITH NO `zip_project_ref` IS NOT A PROJECT TO COLLAPSE. EPA facilities, area
  //    /jurisdiction notices, and every address-mode site carry no project ref. They pass
  //    through ONE FOR ONE - keyed by position, never by a shared falsy key, or the whole
  //    facility rail would collapse into a single row. That is also what makes this function
  //    a safe no-op in address mode.
  // 2. THE SCOPE IS ONE ZIP PAGE. `zip_project_ref` is the project's source_key, and the read
  //    that produced these sites was made for ONE zip (app_zip_projects_markers(p_zip,...)),
  //    so collapsing within a payload can never remove a project from a DIFFERENT ZIP page it
  //    legitimately intersects. Measured: 6,263 of 16,135 road-corridor projects span more
  //    than one ZIP (max 44). Each still appears once per ZIP it intersects.
  // 3. THE REPRESENTATIVE IS DETERMINISTIC. Lowest `zip_marker_seq` wins, and the producer
  //    numbers markers by descending component size, so the representative is the project's
  //    largest component in this ZIP - not "whichever marker the array happened to hold
  //    first". Marker order must never decide what the resident reads.
  HS.zipAuthCollapseToProjects = function (sites) {
    const byRef = Object.create(null);
    const out = [];
    (sites || []).forEach(function (s) {
      const ref = s && s.zip_project_ref;
      if (!ref) { out.push(s); return; }              // rule 1 - not a project marker set
      const prev = byRef[ref];
      if (prev === undefined) { byRef[ref] = out.length; out.push(s); return; }
      const a = out[prev], b = s;                     // rule 3 - lowest marker_seq wins
      const as = (a && a.zip_marker_seq == null) ? Infinity : a.zip_marker_seq;
      const bs = (b && b.zip_marker_seq == null) ? Infinity : b.zip_marker_seq;
      if (bs < as) out[prev] = b;
    });
    return out;
  };
  ...
  HS.zipAuthNote = function (payload, zip, sites) {
    ...
    if (outcome === 'not_measured') {
      // PLAIN LANGUAGE, SAME FACT. This used to end "we will not estimate it from a circle
      // around the ZIP centre" - true, and unreadable to the homeowner it is written for, who
      // has no reason to know what a ZIP centroid is or why we refuse to use one. The load-
      // bearing phrase "not measured yet" is unchanged (the live verifiers key on it) and the
      // next step is stated as an action rather than a caveat.
      return z + ' is not measured yet, so we are not showing development for it. '
           + 'We would rather show nothing than guess. Enter your street address to see what '
           + 'is happening near your home right now.';
    }
    if (outcome === 'unavailable') {
      return 'We could not load development for ' + z + ' just now. This is a temporary '
           + 'problem on our side, not a statement that the ZIP has nothing in it.';
    }
    const n = HS.zipAuthProjectCount(sites);
    if (n === 0) {
      // A REAL ZERO AND A FAILED SEARCH LOOK THE SAME UNLESS THE PAGE SAYS WHICH. Says what
      // was checked before what was found. "whole of ZIP" is the phrase the verifiers key on.
      return 'We checked the whole of ' + z + ' and found no development records. '
           + 'That is a real zero for this ZIP, not a search that came up empty.';
    }
