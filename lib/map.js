...
(function () {
  ...
  const MapProvider = {
    ...
    render(el, opts) {
      ...
      // white halo so it stays readable over anything underneath.
      const homeMark = showHome ? `
        ...
        <text x="${cx}" y="${cy+34}" font-size="11.5" fill="#16211c" stroke="#fff" stroke-width="4" paint-order="stroke" text-anchor="middle" font-weight="700" font-family="sans-serif">Your home</text></g>` : '';
    ...
    function leaflet() {
      HS.loadLeaflet(function (ok) {
        ...
        try {
          ...
          if (o.home) L.marker([o.home.lat, o.home.lng], { zIndexOffset: 1000, icon: div(HS.homeMarkerHTML(22), 22) }).addTo(m);
