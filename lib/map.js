...
(function () {
  ...
  // Splitting the key would have forked that contract into two filter models.
  ...
  const TYPE_FILTER_KEYS = CATEGORY_FILTER_KEYS.filter(function (k) {
  ...
  // A regulated facility whose OWN class fields map to a project Type. Class fields
  // only — the same evidence bar as statedDataCenter(..., classOnly): a passing
  // mention in the free-text name is not a Type. FALLBACK / TERMINAL_NEUTRAL / the
  // facility key itself are not mapped Types, so those records keep the standalone
  // purple square (they have no project symbol for the R to ride on).
  function mappedTypeForFacilityOverlay(item) {
    if (terminalNeutral(item)) return null;
    const display = (item && (item.type || item.use_type || item.layer || item.category)) || '';
    const fields = [item && item.type, item && item.use_type, item && item.layer, item && item.category];
    function accept(info) {
      if (!info || !info.typeKey) return null;
      if (info.typeKey === 'facility' || info.typeKey === 'other') return null;
      const c = CATEGORY_REGISTRY[info.typeKey];
      if (!c || c.isFacility) return null;
      const rule = String(info.shapeRule || '');
      if (rule.indexOf('FALLBACK:') === 0 || rule.indexOf('TERMINAL_NEUTRAL:') === 0) return null;
      // Resident-facing Type name is the legend label, never the raw class
      // field. Tracker rows stamp `layer:'industrial'`; golden rows stamp
      // `type:'industrial'`. Both must read "Industrial", not "industrial".
      info.typeLabel = c.label;
      return info;
    }
    for (let i = 0; i < fields.length; i++) {
      const hit = accept(typeInfoFromExact(fields[i], i === 0 ? display : (fields[0] || fields[i])));
      if (hit) return hit;
      const layerKey = normType(fields[i]);
      const layerHit = LAYER_EXACT[layerKey];
      if (layerHit) {
        const info = accept(Object.assign({
          typeLabel: display || layerHit.legendLabel,
          shapeRule: 'LAYER_EXACT:' + layerKey
        }, layerHit));
        if (info) return info;
      }
    }
    ...
    for (let j = 0; j < KEYWORD_RULES.length; j++) {
      const rule = KEYWORD_RULES[j];
      if (!rule.re.test(combined)) continue;
      const typeKey = rule.typeKey === 'utility' ? 'infrastructure' : rule.typeKey;
      const info = accept({
        typeKey: typeKey,
        shape: (CATEGORY_REGISTRY[typeKey] || {}).symbol,
        legendLabel: (CATEGORY_REGISTRY[typeKey] || {}).label,
        typeLabel: display || (CATEGORY_REGISTRY[typeKey] || {}).label,
        shapeRule: 'KEYWORD:' + typeKey
      });
      if (info) return info;
    }
    return null;
  ...
  function popupLabelFor(item, m) {
    ...
    if (m.dualDataCenter || m.dualOverlay) {
      const typeName = m.typeLabel || (m.dualDataCenter ? CATEGORY_REGISTRY.datacenter.label : '');
      return name + (typeName ? ', ' + typeName : '') + ' · ' + CATEGORY_REGISTRY.facility.label + dist;
    }
  ...
  HS.resolveMarker = function (item, opts) {
    ...
    if (isFacilityItem(item)) {
      ...
      // ── OVERLAY ON TYPE (2026-09-07) ─────────────────────────────────────
      // Regulatory is a third dimension, not an eighth Type. A facility whose own
      // class fields already name Industrial / Energy / Residential / … has a
      // project Type to draw; the purple R rides on that Type the same way it
      // rides on a data-centre octagon. Painting those records as a standalone
      // purple square made Regulatory look like its own kind of facility.
      //
      // MEMBERSHIP stays `facility` only. Adding Industrial (etc.) would leave
      // EPA plants on the project map when Regulatory is off, mixing the two
      // data planes. Data-centre dual identity keeps both memberships because
      // that record IS a data centre. Filter key stays `facility` so the
      // Regulatory chip still owns show/hide.
      const overlayType = mappedTypeForFacilityOverlay(item);
      if (overlayType) {
        const typeCat = CATEGORY_REGISTRY[overlayType.typeKey];
        const ovColor = (opts.colorMode === 'lifecycle' && HS.LIFECYCLE_HEX.operating)
          ? HS.LIFECYCLE_HEX.operating : STATUS_TIERS.operating.hex;
        return {
          shape: typeCat.symbol,
          color: ovColor,
          categoryKey: overlayType.typeKey,
          lifecycle: 'operating',
          lifecycleLabel: STATUS_TIERS.operating.label,
          fallbackReason: null,
          typeKey: overlayType.typeKey,
          typeLabel: typeCat.label,
          statusKey: 'facility',
          statusLabel: CATEGORY_REGISTRY.facility.label,
          legendLabel: typeCat.label,
          shapeRule: 'DUAL:' + (overlayType.shapeRule || overlayType.typeKey) + '+facility',
          popupLabel: popupLabelFor(item, {
            isFacility: true, dualOverlay: true,
            typeLabel: typeCat.label
          }),
          filterKey: 'facility',
          categories: ['facility'],
