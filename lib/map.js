...
(function () {
  ...
  HS.recentChanges = function (projects, changes, meetings, o) {
    ...
    // Fold a change into its related project so the same story doesn't appear twice
    // (e.g. proj-datacenter NEW/HEARING + chg-datacenter UPDATE).
    const byProject = {};
    out.forEach(function (e) { if (e.kind === 'project') byProject[e.id] = e; });
    let folded = out.filter(function (e) {
    ...
    // Multiple agenda/notice rows for the same meeting title (recurring sessions) → one card.
    const merged = [];
    const changeGroups = {};
    folded.forEach(function (e) {
      if (e.kind !== 'change') { merged.push(e); return; }
      const title = String((e.item && (e.item.title || e.item.name)) || '').trim().toLowerCase();
      const key = 'chg:' + (title || e.id);
      if (!changeGroups[key]) { changeGroups[key] = e; merged.push(e); return; }
      const g = changeGroups[key];
      e.badges.forEach(function (b) { if (g.badges.indexOf(b) === -1) g.badges.push(b); });
      e.lines.forEach(function (l) { if (g.lines.indexOf(l) === -1) g.lines.push(l); });
      if (String(e.when || '') > String(g.when || '')) g.when = e.when;
    });
