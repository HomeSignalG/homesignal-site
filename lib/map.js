...
(function () {
  ...
  // Unit A4 — a project may occupy SEVERAL places inside one ZIP (a road corridor, a
  // multi-part parcel), so geography is a LIST while the card stays one per project.
  // Returns the authoritative marker points when the delivery contract supplied them,
  // and otherwise the item's own single point — which is exactly what shipped before,
  // so nothing changes while the authoritative cutover is OFF.
  HS.markerPoints = function (item) {
    if (!item) return [];
    var mk = item._markers;
    if (mk && mk.length) {
      var out = [];
      for (var i = 0; i < mk.length; i++) {
        if (mk[i] && mk[i].lat != null && mk[i].lng != null) {
          out.push({ lat: mk[i].lat, lng: mk[i].lng, marker_seq: mk[i].marker_seq || (i + 1) });
        }
      }
      return out;
    }
    if (item.lat == null || item.lng == null) return [];
    return [{ lat: item.lat, lng: item.lng, marker_seq: 1 }];
  };
      ...
      HS.loadLeaflet(function (ok) {
        ...
        try {
          ...
          items.forEach(function (it) {
            ...
            // One card, possibly several places: every point opens the SAME item.
            HS.markerPoints(it).forEach(function (pt) {
              const mk = L.marker([pt.lat, pt.lng], { icon: div('<div style="line-height:0"' + (o.itemClick ? ' data-hs-map-item="" tabindex="0" role="button"' : '') + '>' + HS.markerSVG(m.shape, m.color, '', 20) + '</div>', 20) });
              if (o.itemClick) {
      ...
      try {
        ...
        items.forEach(function (it) {
          ...
          HS.markerPoints(it).forEach(function (pt) {
            ...
            new maplibregl.Marker({ element: d }).setLngLat([pt.lng, pt.lat]).addTo(map);
          });
        });
        if (o.itemClick) {
