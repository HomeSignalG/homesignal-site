...
(function () {
  ...
  const MapProvider = {
    ...
    render(el, opts) {
      ...
      // Flat neutral field + a home-centered crosshair only. The old decorative
      // green/blue "landmass" and "water" blobs sat at fixed pixel coordinates
      // unrelated to the data or the radius ring, so they never shifted when the
      // radius changed — which read as fake. The crossing lines stay because they
      // pass through the home center (cx,cy), so they always anchor to the home.
      const bg = `
