...
(function () {
  ...
  // Popup body for the saved-place pin — says WHICH address is logged.
  ...
  const MapProvider = {
    ...
    render(el, opts) {
      ...
      // When the caller passes homeLabel (the logged street address) it renders under
      // "Saved place", so the diagram says WHICH address is pinned — only ever a saved
      // address, never derived here. Fix 9: never "Your home".
      ...
      const homeMark = showHome ? `
        ...
        <text x="${cx}" y="${cy+34}" font-size="11.5" fill="#16211c" stroke="#fff" stroke-width="4" paint-order="stroke" text-anchor="middle" font-weight="700" font-family="sans-serif">Saved place</text>${homeAddr}</g>` : '';
      ...
      HS.loadLeaflet(function (ok) {
        ...
        try {
          ...
          if (o.home) L.marker([o.home.lat, o.home.lng], { zIndexOffset: 1000, icon: div(HS.homeMarkerHTML(22), 22),
            title: 'Saved place · ' + (HS.homeAddressLine ? HS.homeAddressLine(o.home) : o.home.address || '') }).addTo(m);
    ...
    map.on('load', function () {
      ...
      try {
        ...
        if (o.home) {   // ONLY a real resident home — never a centroid stand-in; on top of items
          ...
          h.title = 'Saved place · ' + (HS.homeAddressLine ? HS.homeAddressLine(o.home) : o.home.address || '');
