...
(function () {
  ...
  HS.categoryVisible = function (item) {
    ...
    const typeCats = cats.filter(function (k) { return !CATEGORY_REGISTRY[k].isFacility; });
    // Case 1 — its own Type chip is on. Decided before regulatory is consulted at all.
    for (let i = 0; i < typeCats.length; i++) if (categoryFilters[typeCats[i]]) return true;
    if (cats.length === typeCats.length) return false;     // no regulatory membership
    if (!categoryFilters[HS.REGULATORY_LEGEND.key]) return false;
    // Case 2 — untyped regulatory record: the switch is its only governor.
    if (!typeCats.length) return true;
    // Case 3 — typed, its Type is off, and the resident has selected no Type at all.
    return !TYPE_FILTER_KEYS.some(function (k) { return categoryFilters[k]; });
  };
