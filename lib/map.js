...
(function () {
  ...
  const DATACENTER_RE = /data\s*cent(?:er|re|e)|hyperscale|server\s*farm/i;
  ...
  // reshape anything outside the data-centre vocabulary.
  const DATACENTER_CLASS_FIELDS = ['type', 'type_raw', 'use_type', 'layer', 'category'];
  function statedDataCenter(item) {
    if (!item) return null;
    ...
    if (nm && DATACENTER_RE.test(nm) && !DATACENTER_NOT_RE.test(nm)) {
      return Object.assign(cat('datacenter'), {
        typeLabel: CATEGORY_REGISTRY.datacenter.label,
  ...
  const NAME_RULES = [
    ...
    // contradicted the legend, which is GENERATED from the registry. Symbol uniqueness
    // is an invariant — asserted on real rule OUTPUT by test/maps-rule-output-contract.test.mjs.
    // Reachable only when the DATACENTER phase declined this record — which today means
    // its own street-name guard vetoed the name. Without the same guard here, that veto
    // would be undone one phase later and "1100 DATACENTER RD SFR ADDITION" would still
    // draw an octagon. Same shape as the `\bschool\b` guard two rules down.
    { re: /data\s*center|hyperscale|server\s*farm/i, not: DATACENTER_NOT_RE,
      typeKey: 'datacenter', shape: 'octagon', legendLabel: 'Data center' },
