...
(function () {
  ...
  const SIGNIFICANCE = {
    ...
    ancillary: { key: 'ancillary', label: 'Ancillary work' },
    unknown:   { key: 'unknown',   label: 'Significance not stated' }
  };
  HS.SIGNIFICANCE = SIGNIFICANCE;
  // ANCILLARY — the jurisdiction's own permit class says this is supporting work at a
  // data centre rather than construction of one. ENUMERATED from the classes actually
  // observed in production, never a prefix match: Phoenix's "FP " family would sweep in
  // whatever a future source happens to abbreviate the same way.
  const SIG_ANCILLARY_CLASS_RE = /sign\s+permit|fire\s+alarm|fire\s+pump|lead[-\s]?acid\s+battery|battery\s+system|vehicle\s+access\s+control|fire\s+prevention\s+service\s+request/i;
  // …or the record's own words name an ancillary act outright.
  const SIG_ANCILLARY_NAME_RE = /roof\s+replacement|cooling\s+tower|\bsign\s+permit\b/i;
  ...
  const SIG_MAJOR_VETO_RE = /alterat|renovat|repair|addition|tenant/i;
  ...
  function dataCenterSignificance(item) {
    ...
    if (SIG_ANCILLARY_CLASS_RE.test(cls) || SIG_ANCILLARY_NAME_RE.test(nm)) return SIGNIFICANCE.ancillary;
