// HomeSignal map helpers. TWO renderers share this file:
//   * HS.buildGL / the Maps page's inline GL code — the REAL map: MapLibre GL JS with
//     live raster tiles (Esri World Imagery for satellite, OpenStreetMap for street),
//     keyless and already wired. This is the DEFAULT view for covered ZIPs.
//   * HS.MapProvider.render — the schematic SVG "impact diagram", kept as the graceful
//     fallback when the live map is unavailable (offline / artifact preview) or a tile
//     provider rate-limits. `showHome:false` suppresses the home mark (never-faked rule).
// A contracted tile provider (Mapbox/MapTiler/Esri paid) can swap in at HS.buildGL /
// the page's ensureGL — one place — when map-load volume warrants it (Build C).
(function () {
