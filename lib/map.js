...
(function () {
  ...
  //   2. Exact normalized project type string (TYPE_EXACT)
  //   3. Canonical use_type field
  //   4. layer / category fallback (LAYER_EXACT)
  //   5. Deliberately ordered keyword rules (KEYWORD_RULES — specific before broad)
  //   6. Other project (circle)
  const FACILITY_HEX = '#6f42c1';
  const LEGEND_NEUTRAL_HEX = HX.onfile;
  const TYPE_EXACT = {
    'data center':              { typeKey: 'datacenter',     shape: 'square',   legendLabel: 'Data center' },
    'industrial':               { typeKey: 'industrial',     shape: 'triangle', legendLabel: 'Industrial' },
    'residential':              { typeKey: 'residential',    shape: 'pentagon', legendLabel: 'Residential' },
    'roads & infrastructure':   { typeKey: 'infrastructure', shape: 'diamond', legendLabel: 'Roads & infrastructure' },
    'infrastructure':           { typeKey: 'infrastructure', shape: 'diamond', legendLabel: 'Roads & infrastructure' },
    'commercial':               { typeKey: 'commercial',     shape: 'hexagon',  legendLabel: 'Commercial' },
    'utility':                  { typeKey: 'utility',        shape: 'diamond',  legendLabel: 'Roads & infrastructure' },
    'development':              { typeKey: 'other',          shape: 'circle',   legendLabel: 'Other project' },
    'civic/public':             { typeKey: 'civic',          shape: 'circle',   legendLabel: 'Other project' },
    'unclassified':             { typeKey: 'other',          shape: 'circle',   legendLabel: 'Other project' },
    'regulated facility':       { typeKey: 'facility',       shape: 'square',   legendLabel: 'Regulated facility' }
  };
  const LAYER_EXACT = {
  ...
  const KEYWORD_RULES = [
    ...
    { re: /data\s*center|hyperscale|server\s*farm/i, typeKey: 'datacenter', shape: 'square', legendLabel: 'Data center' },
  ...
  HS.STATUS_LEGEND_ROWS = [
    ...
    { key: 'facility',  hex: FACILITY_HEX, label: 'Regulated facility', squareSwatch: true }
  ];
  ...
  function typeInfoFromExact(raw, displayLabel) {
    ...
    if (!hit) return null;
    return Object.assign({ typeLabel: displayLabel || raw || hit.legendLabel }, hit);
  }
  function classifyProjectType(item) {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      if (hit) return hit;
      const layerHit = LAYER_EXACT[normType(fields[i])];
      if (layerHit) return Object.assign({ typeLabel: display || layerHit.legendLabel }, layerHit);
    }
    ...
    for (let j = 0; j < KEYWORD_RULES.length; j++) {
      ...
      if (rule.re.test(combined)) {
        return { typeKey: rule.typeKey, shape: rule.shape, legendLabel: rule.legendLabel, typeLabel: display || rule.legendLabel };
      }
    }
    return { typeKey: 'other', shape: 'circle', legendLabel: 'Other project', typeLabel: display || 'Other project' };
  }
  ...
  HS.resolveMarker = function (item, opts) {
    ...
    if (isFacilityItem(item)) {
      return {
        ...
        color: FACILITY_HEX,
        typeKey: 'facility',
        typeLabel: 'Regulated facility',
        statusKey: 'facility',
        statusLabel: 'Regulated facility',
        legendLabel: 'Regulated facility',
    ...
    const filterKey = st.k === 'onfile' ? 'operating' : st.k;
    const m = {
  ...
  HS.trackerSiteItem = function (s, frsRidFn) {
    ...
    const bucket = (s && s.type === 'built') ? 'built' : ((s && s.type === 'approved') ? 'approved' : 'proposed');
    return {
      type: (s && s.use_type) || '',
      use_type: s && s.use_type,
      layer: s && s.layer,
      status: bucket === 'built' ? 'Operating' : (bucket === 'approved' ? 'Approved' : 'Proposed'),
  ...
  const MapProvider = {
    ...
    render(el, opts) {
      ...
      const pinSvg = pins.map((p, idx) => {
        ...
        const click = itemClick ? (' data-hs-map-item="" tabindex="0" role="button"') : '';
        const hoverParts = HS.markerHoverParts(p.item);
        const hoverAttr = ' aria-label="' + HS.esc(hoverParts.type + '. ' + hoverParts.name) + '"';
        return `<g class="hspin"${click}${hoverAttr} data-pin-idx="${idx}">${shapeEl(shape, p.x, p.y, 15, p.color, 3)}
  ...
  // ── CANONICAL MARKER INTERACTION (hover tooltip + click → sidebar) ─────────
  // ONE hover renderer and ONE click path for every map surface. Hover shows ONLY
  // facility/project type + name. Click opens/updates the page's right sidebar —
  // never navigates, never opens a second popup.
  function markerDisplayName(item) {
    return String((item && (item.name || item.title || item.label)) || 'Project');
  }
  function markerDisplayType(item) {
    var m = HS.resolveMarker(item);
    return m.typeLabel || (item && (item.type || item.use_type || item.category)) || m.legendLabel || 'Project';
  }
  HS.markerHoverParts = function (item) {
    return { type: markerDisplayType(item), name: markerDisplayName(item) };
  };
  HS.markerHoverText = function (item) {
    var p = HS.markerHoverParts(item);
    return p.type + '\n' + p.name;
  };
  HS.markerHoverHTML = function (item) {
    var p = HS.markerHoverParts(item);
    return '<div class="hs-mtip-ty">' + HS.esc(p.type) + '</div>'
      + '<div class="hs-mtip-nm">' + HS.esc(p.name) + '</div>';
  };
  HS.markerHoverForbidden = /\b(status|distance|quality|impact|address|filing|source|document|violat|mi away|from home|operating|proposed|approved)\b/i;
  HS.markerHoverIsClean = function (text) {
    return !!(text && !HS.markerHoverForbidden.test(text));
  };
  HS.attachMarkerHover = function (el, item) {
    if (!el || !item) return;
    var p = HS.markerHoverParts(item);
    el.title = p.type + '\n' + p.name;
    el.setAttribute('aria-label', p.type + '. ' + p.name);
  };
  HS.createMarkerTooltipEl = function () {
    var el = document.createElement('div');
    el.className = 'hs-marker-tip';
    el.setAttribute('role', 'tooltip');
    el.style.cssText = 'position:absolute;z-index:6;display:none;pointer-events:none;max-width:200px;'
      + 'background:rgba(255,255,255,.97);border:1px solid #d5ddd8;border-radius:8px;padding:6px 10px;'
      + 'font:12px/1.35 var(--font,sans-serif);box-shadow:0 2px 8px rgba(0,0,0,.15)';
    return el;
  };
  HS.showMarkerTooltip = function (tipEl, item, clientX, clientY, containerEl) {
    if (!tipEl || !item || !containerEl) return;
    tipEl.innerHTML = HS.markerHoverHTML(item);
    tipEl.style.display = 'block';
    var rect = containerEl.getBoundingClientRect();
    tipEl.style.left = Math.max(6, Math.min(clientX - rect.left + 12, rect.width - 210)) + 'px';
    tipEl.style.top = Math.max(6, Math.min(clientY - rect.top + 12, rect.height - 60)) + 'px';
  };
  HS.hideMarkerTooltip = function (tipEl) {
    if (tipEl) tipEl.style.display = 'none';
  };
  HS.createMapSidebar = function (opts) {
    opts = opts || {};
    var selected = null;
    var panelEl = typeof opts.panel === 'string' ? document.getElementById(opts.panel) : opts.panel;
    var slideEl = typeof opts.slide === 'string' ? document.getElementById(opts.slide) : opts.slide;
    function open() {
      if (slideEl) {
    ...
    function select(item) {
      if (!item) return;
      selected = item;
      if (opts.onSelect) opts.onSelect(item, panelEl);
      open();
      if (panelEl) panelEl.scrollTop = 0;
    }
    function handleMarkerClick(item, ev) {
      if (ev) {
    ...
    function bindMarker(el, item) {
      if (!el || !item) return;
      HS.attachMarkerHover(el, item);
      if (!el.getAttribute('role')) el.setAttribute('role', 'button');
      if (!el.hasAttribute('tabindex')) el.tabIndex = 0;
      var go = function (e) { handleMarkerClick(item, e); };
    ...
    return {
      ...
      setSelected: function (it) { selected = it; },
      open: open,
      close: close,
      select: select,
