...
(function () {
  ...
  HS.resolveMarker = function (item, opts) {
    ...
    if (isFacilityItem(item)) {
      ...
      // `type:'logistics'` → Industrial) draws Type shape + operating colour +
      // purple R. Membership includes the Type key so Regulatory OFF leaves the pin.
      const overlay = classifyFacilityOverlayType(item);
      if (overlay) {
        const overlayColor = (opts.colorMode === 'lifecycle' && HS.LIFECYCLE_HEX.operating)
          ? HS.LIFECYCLE_HEX.operating : STATUS_TIERS.operating.hex;
        return {
          shape: overlay.shape,
          color: overlayColor,
  ...
  const MapProvider = {
    ...
    render(el, opts) {
      const { home, items = [], radiusMi = 1.5, showRadius = true, showHome = true, homeLabel = '', homeTitle = 'Your home', w = 780, h = 520, itemClick } = opts;
      ...
      const homeMark = showHome ? `
        ...
        <text x="${cx}" y="${cy+34}" font-size="11.5" fill="#16211c" stroke="#fff" stroke-width="4" paint-order="stroke" text-anchor="middle" font-weight="700" font-family="sans-serif">${HS.esc(homeTitle)}</text>${homeAddr}</g>` : '';
    ...
    function schematic() {
      try {
        MapProvider.render(el, { home: center, items: items, radiusMi: o.radiusMi || 1.5,
          ...
          homeTitle: o.homeCaption || 'Your home',
    ...
    function leaflet() {
      HS.loadLeaflet(function (ok) {
        ...
        try {
          ...
          const homeCaption = o.homeCaption || 'Your home';
          if (o.home) L.marker([o.home.lat, o.home.lng], { zIndexOffset: 1000, icon: div(HS.homeMarkerHTML(22), 22),
            title: homeCaption + ' · ' + (HS.homeAddressLine ? HS.homeAddressLine(o.home) : o.home.address || '') }).addTo(m);
          ...
          if (o.fitItems) {
            const b = items.map(function (it) { return [it.lat, it.lng]; });
            if (o.home) b.push([o.home.lat, o.home.lng]);
            if (b.length) try { m.fitBounds(b, { padding: [28, 28], maxZoom: 13 }); } catch (e) { /* keep setView */ }
          }
    ...
    map.on('load', function () {
      ...
      try {
        ...
        if (o.home) {   // ONLY a real resident home — never a centroid stand-in; on top of items
          ...
          const homeCaption = o.homeCaption || 'Your home';
          h.title = homeCaption + ' · ' + (HS.homeAddressLine ? HS.homeAddressLine(o.home) : o.home.address || '');
        ...
        if (o.fitItems && items.length) {
          ...
          try { map.fitBounds([[minLng, minLat], [maxLng, maxLat]], { padding: 40, maxZoom: 13 }); } catch (e) { /* keep constructor center */ }
        }
  ...
  // Place-page Map 1: Address gets the house logo at the saved point; ZIP Code is
  // geography (records' own extent, no home pin, no radius). Same backbone as the
  // Dashboard preview. Callers must never pass a sample/demo/centroid as `home`.
  HS.placeMap = function (el, o) {
    if (!el) return;
    o = o || {};
    const isAddr = o.mode === 'address';
    const raw = o.home;
    const home = (isAddr && raw && raw.lat != null && raw.lng != null
      && !raw.sample && !raw.demo) ? raw : null;
    const items = (o.items || []).filter(function (it) {
      return it && it.lat != null && it.lng != null;
    }).slice(0, 64);
    const center = home
      || (o.center && o.center.lat != null && o.center.lng != null ? o.center : null)
      || (items[0] ? { lat: items[0].lat, lng: items[0].lng } : null);
    function go() { if (o.mapHref) location.href = o.mapHref; }
    HS.buildLive(el, {
      center: center,
      items: items,
      home: home,
      homeCaption: o.homeCaption || (home ? 'Address' : ''),
      interactive: true,
      zoom: isAddr ? 14 : 12,
      radiusMi: (isAddr && home) ? 1.5 : undefined,
      fitItems: !isAddr && items.length > 1,
      w: o.w || 640,
      h: o.h || 280,
    ...
    el.addEventListener('click', function (e) {
      if (el._hsDragged || (e.target && e.target.closest && e.target.closest('[data-hs-map-item]'))) return;
      go();
    });
    el.addEventListener('keydown', function (e) {
      if (e.key === 'Enter' && !(e.target && e.target.closest && e.target.closest('[data-hs-map-item]'))) go();
    });
  };
})();
