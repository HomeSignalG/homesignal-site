...
(function () {
  ...
  // GeoJSON for the GL clustered source: one point per record, colored by its
  // resolved marker status color, carrying the id for the click -> panel dispatch
  // and a facility flag so facility points route to the facility detail.
  HS.restFeatureCollection = function (items) {
    return { type: 'FeatureCollection', features: (items || []).map(function (it) {
      var mk = HS.resolveMarker ? HS.resolveMarker(it) : { color: '#6b7f76' };
      return { type: 'Feature',
        geometry: { type: 'Point', coordinates: [+it.lng, +it.lat] },
        properties: { id: it.id, col: mk.color, fac: it._restFacility ? 1 : 0 } };
    }) };
  };
  ...
  const GENERIC_EXACT = new Set(['development', 'unclassified', 'trades', 'land use', 'civic/public', 'civic']);
  ...
  const NAME_RULES = [
    ...
    { re: /wireless\s+communication|cell\s+tower|antenna|water\s+main|sewer\s+main|gas\s+main\b|right[-\s]?of[-\s]?way|\broadway\b|street\s+improvement/i,
      typeKey: 'infrastructure', shape: 'diamond', legendLabel: 'Roads & infrastructure' },
    { re: /industrial|warehouse|manufactur|factory/i, typeKey: 'industrial', shape: 'triangle', legendLabel: 'Industrial' },
    { re: /residential|\b1,\s?2,\s?3\s+family\b|\b(one|two|three|single|multi|1|2|3)[-\s]?family\b|townhou?se|duplex|dwelling|apartment|condominium/i,
      typeKey: 'residential', shape: 'pentagon', legendLabel: 'Residential' },
    { re: /commercial/i, typeKey: 'commercial', shape: 'hexagon', legendLabel: 'Commercial' }
  ];
  HS.SHAPE_LEGEND = [
  ...
  function classifyProjectType(item) {
    ...
    return { typeKey: 'other', shape: 'circle', legendLabel: 'Other project',
             typeLabel: display || 'Other project', shapeRule: 'FALLBACK:other' };
  }
