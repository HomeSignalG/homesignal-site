      ...
      HS.loadLeaflet(function (ok) {
        ...
        try {
          ...
          items.forEach(function (it) {
            L.marker([it.lat, it.lng], { icon: div('<div style="line-height:0">' + HS.markerSVG(HS.projectShape(it), statusTier(it).hex, '', 20) + '</div>', 20) }).addTo(m);
          });
        } catch (e) { schematic(); }
      });
    }
    ...
    map.on('load', function () {
      ...
      try {
        ...
        items.forEach(function (it) {
          ...
          d.style.cssText = 'width:20px;height:20px;line-height:0';
