...
(function () {
  ...
  // The KEYS, LABELS and status → key rule live in lib/project-type.js (moved 2026-09-24 so
  // the ZIP page's facility cards say the lifecycle in the same words without loading this
  // file). This file adds only the COLOUR. Fail loud if the vocabulary is missing.
  const LC = HS.projectType;
  if (!LC || !LC.LIFECYCLE_LABELS || typeof LC.lifecycleKey !== 'function') {
    throw new Error('lib/map.js requires lib/project-type.js to be loaded first (the canonical lifecycle vocabulary).');
  }
  const LIFECYCLE_KEYS = LC.LIFECYCLE_KEYS.slice();
  const STATUS_TIERS = {
    proposed:  { hex: HX.proposed,  label: LC.LIFECYCLE_LABELS.proposed },
    approved:  { hex: HX.approved,  label: LC.LIFECYCLE_LABELS.approved },
    operating: { hex: HX.operating, label: LC.LIFECYCLE_LABELS.operating },
    unknown:   { hex: HX.onfile,    label: LC.LIFECYCLE_LABELS.unknown }
  };
