  ...
  const MapProvider = {
    ...
    render(el, opts) {
      ...
      const pinSvg = pins.map(p => {
        ...
        return `<g>${shapeEl(shape, p.x, p.y, 15, p.color, 3)}
      ...
      HS.loadLeaflet(function (ok) {
        ...
        try {
          ...
          items.forEach(function (it) {
            const mk = L.marker([it.lat, it.lng], { icon: div('<div style="line-height:0"' + (o.itemClick ? ' data-hs-map-item=""' : '') + '>' + HS.markerSVG(HS.projectShape(it), statusTier(it).hex, '', 20) + '</div>', 20) });
            if (o.itemClick) {
              mk.on('click', function (e) { if (e.originalEvent) e.originalEvent.stopPropagation(); o.itemClick(it); });
            }
            mk.addTo(m);
          });
        } catch (e) { schematic(); }
    ...
    map.on('load', function () {
      ...
      try {
        ...
        items.forEach(function (it) {
          ...
          if (o.itemClick) d.setAttribute('data-hs-map-item', '');
          d.innerHTML = HS.markerSVG(HS.projectShape(it), statusTier(it).hex, '', 20);
          if (o.itemClick) {
            d.addEventListener('click', function (e) { e.stopPropagation(); o.itemClick(it); });
          }
