...
(function () {
  ...
  HS.whyDerive = function (it, ctx) {
    ...
    if (hasDist && ctx.radiusMi != null && it.distance_mi <= ctx.radiusMi) {
      seeing = 'It’s inside your ' + ctx.radiusMi + '-mile map view around your saved place.';
    } else if (hasDist) {
      seeing = 'It’s on the public record near your saved place.';
    } else if (ctx.place) {
    ...
    let close;
    if (hasDist) {
      close = 'Located about ' + (it.dist || HS.fmtMi(it.distance_mi)) + ' from your saved place'
      ...
      close = 'It’s shown at its recorded location on the map. Save an address to see how far away it is.';
    }
