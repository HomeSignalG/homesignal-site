...
(function () {
  ...
  HS.whyDerive = function (it, ctx) {
    ...
    // Parties are named with the SOURCE's own role word ("Owner on file: …"), because a
    // filing that says OWNER is not evidence of an applicant. `parties` is the structured
    // form; the legacy single `developer` field is the fallback for records materialised
    // before it existed, and keeps its historical "Applicant on file" wording.
    const named = (Array.isArray(it.parties) ? it.parties : []).filter(function (p) { return p && p.name && p.role; });
    if (named.length) named.forEach(function (p) { know.push(p.role + ' on file: ' + p.name); });
    else if (it.developer) know.push('Applicant on file: ' + it.developer);
