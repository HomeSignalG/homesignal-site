...
// geography, ZIP membership, residential qualification, network calls, or any surface's
// presentation. `symbol` in the registry is a NAME the renderer looks up, not code.
// The LIFECYCLE VOCABULARY (keys, labels, status → key) joined on 2026-09-24 by founder
// decision, for the same reason the Type did — see the section near the end of this file.
// Lifecycle COLOUR still does not belong here.
...
(function () {
  ...
  // ── THE CANONICAL LIFECYCLE VOCABULARY (moved from lib/map.js, 2026-09-24) ──────────────
  // FOUR members, no more: proposed | approved | operating | unknown. `unknown` is a
  // FIRST-CLASS state: a record whose source states no lifecycle must never be promoted to
  // operating (that fabricates a fact) nor demoted to proposed.
  //
  // WHY IT LIVES HERE. The ZIP page's regulated-facility cards must say the lifecycle in the
  // SAME words Map 1 uses, and the ZIP hosts may not load lib/map.js. Founder decision
  // 2026-09-24: extend this existing lightweight authority rather than keep a second
  // vocabulary. Keys, labels and the status → key resolution only. COLOUR stays in
  // lib/templates.js (HS.statusHex) / lib/map.js, as before.
  //
  // STORAGE. The stored representation of "source states no lifecycle" is the status string
  ...
  // For a surface that shows a record's lifecycle without drawing it. Reads the stored
  // `status` only — never the record kind, never a regulatory permit or program status.
  function canonicalLifecycle(row) {
    const k = lifecycleKey(row && row.status);
    return { key: k, label: LIFECYCLE_LABELS[k] };
  }
  HS.canonicalLifecycle = canonicalLifecycle;
