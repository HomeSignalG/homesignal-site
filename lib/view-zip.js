...
(function () {
  ...
  // ---- SAVED PLACES vs VIEWED PLACE (the ownership boundary) ----------------
  // Account hydration synchronizes the followed-ZIP COLLECTION. It must never decide
  // which ZIP is being VIEWED. shell.js::syncFollowsFromAccount used to do both — it
  // assigned `state.zip = serverFollowZips[0]` and overwrote myZip with it on every
  // signed-in boot — so app_follows ROW ORDER silently replaced the place the resident
  // had explicitly chosen (alerts.html?zip=84301 hydrated, then rendered 75009).
  //
  // Given the account's follow list, this returns ONLY the myZip value that should be
  // written, or null to leave myZip alone. It can FILL an absent myZip (first-follow
  // initialization — cross-device onboarding) and can never REPLACE an established one.
  // It has no viewed-ZIP return value at all, which is what makes the boundary
  // structural rather than a convention: a follow list is not a viewed-place fallback.
  function myZipAfterFollowSync(opts) {
    opts = opts || {};
    var cur = opts.myZip;
    if (cur && /^\d{5}$/.test(String(cur))) return null;   // established — never replaced
    var list = (opts.serverFollowZips || []).concat(opts.localFollowZips || []);
    for (var i = 0; i < list.length; i++) {
      var z = list[i] == null ? '' : String(list[i]);
  ...
  // Post-save confirmation identity. SAVE/FOLLOW is My Places, not email alerts.
  // Address identity wins when present (the confirmed/stored address string).
  // ZIP identity reuses the followed community name and does not double the ZIP
  // when the name already carries it ("Celina (75009)").
  function placeSavedLabel(info) {
    info = info || {};
  ...
  // Canonical Alerts routing for the place that was just saved. ZIP context is
  // ?zip= via navHref. Address context rides the existing activeProp identity;
  // this href does not invent a second place-id scheme.
  function alertsHrefForSavedPlace(info, hrefFn) {
    info = info || {};
    var zip = String(info.zip || '').trim();
    hrefFn = hrefFn || navHref;
    return hrefFn('alerts.html', /^\d{5}$/.test(zip) ? zip : null);
  }
  ...
  // ZIP-SCOPED TOOLS ONLY. dashboard.html was REMOVED (Fix 8, founder decision D1): the
  // Dashboard is now an account-wide ALL MY PLACES briefing, so stamping ?zip= on its own
  // link would put one currently-viewed ZIP on a surface that is about every saved place —
  // and HS.submitWaitlist's zipFromLocation fallback would then attribute a feature-level
  // Premium lead to that ZIP. Same reason properties.html (My Places) has always been
  // excluded: an account-wide surface is not a ZIP navigation target.
  var ZIP_NAV_PAGES = ['alerts.html', 'development.html', 'homesignalmap.html', 'community.html'];

  // The two map experiences — cross-links always carry ?zip= via navHref.
  var MAP_PAGES = ['homesignalmap.html'];

  var api = {
    parseZipParam: parseZipParam,
    parseZipFromAddress: parseZipFromAddress,
    resolveViewedZip: resolveViewedZip,
    myZipAfterFollowSync: myZipAfterFollowSync,
    navHref: navHref,
    pageHref: pageHref,
    ...
    MAP_PAGES: MAP_PAGES,
    placeSavedLabel: placeSavedLabel,
    alertsHrefForSavedPlace: alertsHrefForSavedPlace
  };
