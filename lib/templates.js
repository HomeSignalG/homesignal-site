...
(function () {
  ...
  const tpl = {
    ...
    impactScoreLine(p) {
      ...
      const display = val || '—';
      return `<p class="impactline"><b>${QOL_IMPACT_SCORE_BRAND}:</b> ${esc(display)}</p>`;
    },
  ...
  // ── Company ESG / Sustainability (docs/esg-company-layer.md) ───────────────────────
  // Works on app_projects.company_esg — written ONLY by the esg-refresh service, ONLY for
  // a confident company match, ONLY for pilot ZIPs. Absent = "ESG data unavailable".
  //
  // THE ONE RULE THIS LAYER EXISTS TO ENFORCE: this is COMPANY-level information about the
  // business associated with a record. It is NEVER a measurement of the individual facility.
  // Facility-level environmental fact comes from HS.fac (EPA ECHO / TCEQ) and the two must
  // never be blended into one line, one heading, or one score.
  //
  //   • No zero. A missing match renders the words "ESG data unavailable" — never "0", which
  //     a reader would take as "this company scores zero".
  //   • No synthesised score. If the source publishes no scaled overall score, none is shown.
  //   • Role-aware framing: for a PROPOSED / unbuilt development this is the developer's
  //     TRACK RECORD; for an OPERATING facility it is the associated company's profile.
  const ESG_SCOPE_NOTE = 'Company-level sustainability data; not a rating of this individual facility.';
  const ESG_UNAVAILABLE = 'ESG data unavailable';
  const ESG_ROLE_WORD = { developer: 'Developer', operator: 'Site operator', owner: 'Owner', parent: 'Parent company', unknown: 'Associated company' };
  const ESG_PILLAR_LABEL = { environmental: 'Environmental', social: 'Social', governance: 'Governance', unclassified: 'Other reported areas' };
  const ESG_SOURCE_LABEL = { wba: 'World Benchmarking Alliance (via WikiRate)', wikirate: 'WikiRate' };

  HS.esg = {
    SCOPE_NOTE: ESG_SCOPE_NOTE,
    UNAVAILABLE: ESG_UNAVAILABLE,

    /** The stored blob, or null. Anything without a company name is treated as absent. */
    of(row) {
      const e = row && row.company_esg;
      return (e && e.company_name) ? e : null;
    },

    /**
     * THE MAP CARD LINE — deliberately ONE line and NO metrics.
     * A map popup is a glance surface; pillar detail there is the clutter the pilot is
     * meant to rule out, and a half-shown ESG metric is the easiest place for a reader to
     * mistake company data for facility data. Returns { text, available }.
     */
    chip(row) {
      const e = this.of(row);
      if (!e) return { text: ESG_UNAVAILABLE, available: false };
      const who = e.company_name + (e.match_confidence === 'parent' && e.parent_company_name
        ? ' (via ' + e.parent_company_name + ')' : '');
      return { text: 'Company ESG · ' + who, available: true };
    },

    /** "Developer track record" vs "Company profile" — the framing differs by lifecycle. */
    heading(row) {
      const e = this.of(row);
      if (!e) return 'Company ESG / Sustainability';
      const proposed = /proposed|on file|approved/i.test(String(e.record_status || row.status || ''));
      return proposed && e.role === 'developer'
        ? 'Developer sustainability track record'
        : 'Company ESG / Sustainability';
    },

    /** The one-sentence explainer under the heading, in plain consumer language. */
    framing(row) {
      const e = this.of(row);
      if (!e) return '';
      const roleWord = ESG_ROLE_WORD[e.role] || ESG_ROLE_WORD.unknown;
      const proposed = /proposed|on file|approved/i.test(String(e.record_status || row.status || ''));
      return proposed
        ? 'This is the published sustainability record of the company behind this proposal (' +
          roleWord.toLowerCase() + '), not a measurement of the proposed facility.'
        : 'This is the published sustainability record of the company associated with this site (' +
          roleWord.toLowerCase() + '). Environmental conditions AT this facility come from EPA records above.';
    },

    /** Pillar sections that actually have content: [{ key, label, rows }]. */
    sections(row) {
      const e = this.of(row);
      if (!e) return [];
      return ['environmental', 'social', 'governance', 'unclassified']
        .map((k) => ({ key: k, label: ESG_PILLAR_LABEL[k], rows: e[k] || [] }))
        .filter((s) => s.rows.length);
    },

    /**
     * The score line — present ONLY when the source published a score AND its scale.
     * A bare number with no scale is worse than no number: verified 2026-08-09, WikiRate
     * publishes no unit/value_type for the WBA benchmark metrics, so nothing currently
     * qualifies and this returns null. It stays wired so a future sourced scale renders
     * without a code change.
     */
    scoreLine(row) {
      const e = this.of(row);
      if (!e || e.overall_score == null || !e.overall_score_scale) return null;
      return e.overall_score + ' (' + e.overall_score_scale + ')';
    },

    /** Source + licence lines. Attribution is REQUIRED wherever the data renders (CC BY 4.0). */
    provenance(row) {
      const e = this.of(row);
      if (!e) return null;
      const srcs = {};
      this.sections(row).forEach((s) => s.rows.forEach((m) => { srcs[m.source] = 1; }));
      const names = Object.keys(srcs).map((k) => ESG_SOURCE_LABEL[k] || k);
      return {
        sources: names.length ? names : [ESG_SOURCE_LABEL.wikirate],
        reporting_year: e.reporting_year || null,
        attribution: e.attribution || '',
        url: e.source_url || '',
        confidence: e.match_confidence || '',
        matched_name: e.source_company_name || ''
      };
    }
  };
})();
