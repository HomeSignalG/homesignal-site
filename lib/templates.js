...
(function () {
  ...
  const PARENT_ROLES = ['Owner', 'Property Owner', 'Developer', 'Applicant', 'Operator'];
  ...
  const IDENTITY_ROLES = ['Property Owner', 'Developer', 'Applicant', 'Operator'];
  ...
  HS.parties = {
    ...
    verificationWord(v) { return VERIFICATION_WORD[String(v || '').toUpperCase()] || ''; },
    ...
    parent(p) {
      ...
      if (pa && pa.verification === 'verified' && pa.name) {
        return { verified: true, name: pa.name, source: pa.source, url: pa.url,
                 retrieved_at: pa.retrieved_at, attribution: pa.attribution || 'parent_company' };
      }
    ...
    rowsHTML(list) {
      return (list || []).map(p => {
        ...
        let parentHTML = '';
        if (par && par.verified) {
          parentHTML = '<span class="pparent"><span class="pplab">Parent company</span> '
            + '<b>' + esc(par.name) + '</b></span>';
        } else if (par) {
          // An absent line would read as "this company has no parent", which is a claim
          // we have not established. Saying we have not checked is the honest state.
          parentHTML = '<span class="pparent none"><span class="pplab">Parent company</span> '
            + 'Not yet verified</span>';
        }
        const vw = this.verificationWord(p.verification);
        const badge = vw ? ' <span class="pver v' + esc(String(p.verification).toLowerCase()) + '">' + esc(vw) + '</span>' : '';
        ...
        return '<div class="prow"><span class="prole">' + esc(p.role) + '</span>'
          + '<span class="pname"><b>' + esc(p.name) + '</b>' + badge + legal
          + (where ? ' <span class="pmeta">' + esc(where) + '</span>' : '')
          + parentHTML + '</span></div>';
      }).join('');
    },
    // Evidence rows for the Sources & verification disclosure — one per VERIFIED parent.
    // Nothing to show when no relationship is verified, which is the state today.
    evidenceRows(list) {
      const out = [];
      (list || []).forEach(p => {
        // A resolved role carries the source that states it. Rows with no source are the
        // legacy as-filed parties, whose evidence IS the filing already linked on the card.
        if (p && p.source) {
          out.push({
            label: p.role + ' — ' + p.name,
            value: p.source + (p.verification ? ' (' + (this.verificationWord(p.verification) || p.verification) + ')' : ''),
            url: p.url || ''
          });
        }
        const par = this.parent(p);
        if (!par || !par.verified) return;
        out.push({
          label: 'Parent of ' + p.name,
          value: par.name + (par.source ? ' — ' + par.source : ''),
          url: par.url || ''
        });
      });
      return out;
