...
(function () {
  ...
  // Permit-status precedence, most-active first (the FAC_STATUS key order). Drives
  // mostActive() when co-located registrations disagree (e.g. Expired beats Terminated).
  const FAC_STATUS_ORDER = Object.keys(FAC_STATUS);
  ...
  HS.fac = {
    ...
    // ── Co-located same-name facilities (multiple EPA registrations at one site) ──
    // A single physical site can hold more than one FRS registration (e.g. two permits
    // with different statuses). They must render as ONE card but keep BOTH records
    // reachable. groupKey merges by name + location (lat/lng to 3 dp ≈ same site —
    // anchor-independent, i.e. "same distance from anywhere"); non-facilities never merge.
    groupKey(f) {
      if (!f || f.record_kind !== 'facility') return null;
      const nm = String(f.name || '').trim().toUpperCase();
      const la = (f.lat != null) ? Number(f.lat).toFixed(3) : '?';
      const ln = (f.lng != null) ? Number(f.lng).toFixed(3) : '?';
      return nm + '@' + la + ',' + ln;
    },
    coLocated(f, list) {
      const k = this.groupKey(f);
      if (!k) return f ? [f] : [];
      return (list || []).filter(x => this.groupKey(x) === k);
    },
    // The most-ACTIVE registration in a group drives the merged card's one interpreted
    // line — so the card never implies the whole site is Terminated when another
    // registration is still active. Unknown/absent status sorts after every known one.
    mostActive(group) {
      if (!group || !group.length) return null;
      let best = group[0], bestRank = 99;
      group.forEach(g => {
        const st = g.facility_env && g.facility_env.epa && g.facility_env.epa.permit_status;
        const r = (st && FAC_STATUS_ORDER.indexOf(st) >= 0) ? FAC_STATUS_ORDER.indexOf(st) : 98;
        if (r < bestRank) { bestRank = r; best = g; }
      });
      return best;
    },
    // Collapse each co-located same-name facility group to one representative (the
    // most-active member), stamped `_mergeCount`. Order-preserving (rep takes the
    // first member's slot — the closest, since the input is proximity-sorted). Used by
    // every facility-card surface so the same merge shows everywhere.
    dedupe(list) {
      const seen = {}, out = [];
      (list || []).forEach(x => {
        if (x && x.record_kind === 'facility') {
          const k = this.groupKey(x);
          if (k) {
            if (seen[k]) return;
            seen[k] = 1;
            const group = this.coLocated(x, list);
            if (group.length > 1) { out.push(Object.assign({}, this.mostActive(group), { _mergeCount: group.length })); return; }
          }
        }
        out.push(x);
      });
    ...
    // Small secondary marker telling the reader a card is a merge (N registrations),
    // WITHOUT displacing the interpreted line — the line does the work, this just flags it.
    mergeNote(f) {
      const n = f && f._mergeCount;
      return (n > 1) ? '<span style="color:var(--ink-3);font-weight:400"> · ' + n + ' EPA registrations</span>' : '';
    }
  };
  ...
  tpl.facCard = function (f) {
    return '<div class="card clickable" style="border-left-color:#6f42c1" '
      ...
      + '<span class="lens">' + esc(HS.fac.program(f)) + (f.dist ? ' · ' + esc(f.dist) : '') + HS.fac.mergeNote(f) + '</span>'
      + '<h3>' + esc(f.name) + '</h3>'
