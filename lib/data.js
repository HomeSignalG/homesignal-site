    ...
    const p = (async () => {
      try {
        const { data, error } = await sb().from('v_app_project_identity')
          .select('project_id,identity,frs').eq('zip', zip);
        if (error || !Array.isArray(data)) return new Map();
        const m = new Map();
        // Two independent enrichments on one row: the resolved identity layer and the EPA
        // FRS affiliations. Either may be empty on its own, so a record is attached when
        // EITHER has content — otherwise a facility whose only party information is an FRS
        // affiliation would keep the "not yet available" empty state.
        for (const r of data) {
          ...
          if (ident || hasFrs) m.set(r.project_id, { identity: ident, frs: hasFrs ? f : null });
        }
        return m;
