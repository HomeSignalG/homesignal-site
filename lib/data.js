...
(function () {
  ...
  const data = {
    ...
    async meetings(zip, home) {
      ...
      if (!c) return [];
      // Content anchors at the chain ROOT (the county today), so a page must query EVERY
      // ancestor's community_id — not just its immediate parent. Walk the full parent chain
      // (ZIP → city → county → …). For every existing 2-level chain this yields exactly the
      // old [self, parent]; only a 3-level page (a ZIP under an incorporated city, e.g.
      // Brigham City 84302) gains the county id it needs to see county-anchored meetings.
      const ids = [c.id];
      let pid = c.parent_id, hops = 0;
      while (pid && hops++ < 6) {   // hop cap guards against a parent_id cycle
        ids.push(pid);
        const { data: up } = await sb().from('communities').select('parent_id').eq('id', pid).limit(1);
        pid = up && up[0] ? up[0].parent_id : null;
      }
