...
(function () {
  ...
  // UNAVAILABLE IS NOT A FAILURE, AND NEITHER IS IT A ZERO. `app_projects_for_zip` answers a
  // ZIP-development read with an OBJECT rather than an array when the ZIP's current geography
  // state cannot support one - `not_measured` (the pinned TIGER contract has no ZCTA for this
  // ZIP, so whole-ZIP measurement never happened) or pending. That is a definite answer, so it
  // is NOT retried, and it is reported as `unavailable` carrying the status verbatim rather
  // than being flattened into the same `complete:false` a network failure produces. The two
  // must stay apart: one means "we could not read", the other means "there is nothing here to
  // read, and saying zero would be a lie".
    ...
    for (let attempt = 0; attempt < 2; attempt++) {          // one retry, as fetchAllPages does
      ...
      if (!error && data && typeof data === 'object' && data.unavailable === true) {
        return { rows: [], complete: false,
                 unavailable: String(data.zip_geography_status || 'unknown') };
      }
    }
    return { rows: [], complete: false };
  }
  ...
  // ONE SENTENCE, ONE PLACE. Every surface that renders ZIP development says the same thing
  // when the whole-ZIP read is unavailable, so two pages cannot describe the same state
  // differently - which is the defect this whole unit exists to end. The wording deliberately
  // matches Map 1's established not-measured copy: it names the reason, refuses to estimate
  // from a circle around the ZIP centre, and points at the surfaces that ARE unaffected.
  // ONE PLACE DECIDES WHETHER A ZIP'S DEVELOPMENT COUNT IS A MEASUREMENT AT ALL.
  // Every surface that consumes `projects()` renders a NUMBER somewhere - a stat tile, a
  // "Projects tracked" vital, a "Nearby projects" count - and a number is an assertion. Printing
  // 0 for a ZIP whose whole-ZIP geography was never measured states a finding HomeSignal did not
  // make; that is the same false zero this unit removed from the prose, arriving through the
  // arithmetic instead. So the count is rendered only when it means something, and an em dash
  // otherwise, with the surrounding copy carrying the reason.
  HS.zipDevelopmentState = function (projects) {
    if (!projects) return 'read_failed';
    if (projects.unavailable) return String(projects.unavailable);
    if (projects.complete === false) return 'read_failed';
    return null;                       // measured - the count means exactly what it says
  };

  HS.zipDevelopmentCount = function (projects, count) {
    if (HS.zipDevelopmentState(projects)) return '\u2014';
    return count != null ? count : (projects ? projects.length : 0);
  };

  HS.zipDevelopmentUnavailableNote = function (status, zip) {
    var z = zip ? ('ZIP ' + zip) : 'this ZIP';
    if (status === 'not_measured') {
      return 'Whole-ZIP development measurement is not available for ' + z + ' — it has no ZCTA '
           + 'boundary in the Census geography HomeSignal measures against, so there is nothing '
           + 'to measure and we will not estimate it from a circle around the ZIP centre. This '
           + 'is not a finding of zero. Regulated facilities below are unaffected.';
    }
    if (status === 'boundary_complete_not_cut_over') {
      return 'Whole-ZIP development measurement for ' + z + ' is complete but not published here '
           + 'yet. Rather than estimate, we are showing nothing until it is.';
    }
    if (status === 'read_failed') {
      // A TIMED-OUT READ IS NOT A ZERO, and this is the door the defect comes back through.
      // Measured 2026-09-06 under sustained load on this RPC (18 active queries, 13 of them
      // here, longest 53 s): 28456 returned its correct 12 rows in 21,132 ms, far past the
      // caller's budget. The page then had an empty array and said "no permit or planning
      // records on file" — a false measured zero for a perfectly healthy ZIP. Failure and
      // emptiness must never render the same.
      return 'Development records for ' + z + ' could not be read just now. This is a temporary '
           + 'read failure, not a finding of zero — reload in a moment. Regulated facilities '
           + 'below are unaffected.';
    }
    return 'Whole-ZIP development measurement for ' + z + ' is not resolved yet — we will not '
         + 'estimate it from a circle around the ZIP centre. This is not a finding of zero. '
         + 'Regulated facilities below are unaffected.';
  };
  ...
  const data = {
    ...
    async projects(zip, home) {
      ...
      // Rides alongside `complete` for the same reason it does: a page that renders an empty
      // development section must be able to say WHY it is empty. Absent = the read succeeded
      // and the ZIP genuinely holds no qualifying records (a measured zero).
      out.unavailable = res.unavailable || null;
      return out;
    },
