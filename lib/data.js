  ...
  const data = {
    ...
    async projects(zip, home) {
      ...
      const { data } = await sb().from('app_projects').select('*')
        .eq('zip', zip).eq('record_kind', 'development').order('impact_score', { ascending: false });
    ...
    async properties() {
      ...
      if (CFG.DEMO_SESSION && (!HS.state || !HS.state.session || HS.state.session.demo)) {
        // Demo sample homes on the LIVE site are FICTIONAL addresses — flag them so
        // public record pages can refuse to measure real records from a made-up house.
