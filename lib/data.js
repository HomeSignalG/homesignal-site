...
(function () {
  ...
  const data = {
    ...
    async news(zip, home, follows) {
      ...
      // `follows` (optional) = the user's saved News-tier topic labels; when present the
      // rows are filtered by app_changes.subtopics overlap (Phase B). Absent/empty ->
      // ALL Local News, preserving the prior behavior for anonymous / no-selection users.
      // subtopics survives select('*') + normChange (Object.assign keeps every column).
      zip = zip || CFG.DEFAULT_ZIP;
      home = homeFor(zip, home);
      const filt = (window.HS.topicPrefsUtil && window.HS.topicPrefsUtil.filterNewsByTopics)
        || function (items) { return items; };
      if (isSeed()) {
        const seed = withDistance((window.HS_SEED.changes || []).filter(function (c) { return c.category === 'Local News'; }), home);
        return filt(seed, follows);
      }
      const { data } = await sb().from('app_changes').select('*')
        .eq('zip', zip).eq('category', 'Local News').order('occurred_at', { ascending: false });
      return filt(withDistance((data || []).map(normChange), home), follows);
    },
