...
(function () {
    ...
    const p = (async () => {
      try {
        const { data, error } = await sb().from('v_app_project_identity')
          .select('project_id,identity').eq('zip', zip);
        if (error || !Array.isArray(data)) return new Map();
        const m = new Map();
        for (const r of data) if (r && r.project_id && Array.isArray(r.identity) && r.identity.length) m.set(r.project_id, r.identity);
        return m;
  ...
  function attachIdentity(rows, map) {
    ...
    for (const r of rows) { const v = map.get(r.id); if (v) r.identity = v; }
    return rows;
  }
