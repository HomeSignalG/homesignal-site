...
// DATA-QUALITY GATE (feed-coverage truth): community() returns data_quality
// ('pass' | 'wired_silent' | 'coverage_coming') plus feed_coverage — the per-tab
// bucket map {gov_notices|meetings|local_news: 'no_feed'|'wired_silent'|'producing'}
// derived daily from feed_coverage_check in the engine. Pages render honest
// per-tab states from it; only 'pass' pages are ever indexable.
(function () {
  ...
  const data = {
    ...
    async coverageStatus(zip) {   // 'pass' | 'wired_silent' | 'coverage_coming' | null(not covered)
