...
(function () {
  ...
  // ------------------------------------------------------- sign-out hygiene --
  // The localStorage keys that hold ONE account's personalized state. On sign-out
  // these MUST be cleared, or the next visitor on the same browser inherits the
  // signed-out chrome of the account that just left (a cross-account privacy leak):
  // their saved ZIP, active property id, follows, topic picks, dismissed cards and
  // followed communities would all persist. 'referral' is deliberately EXCLUDED —
  // it's browser-level marketing attribution, not account state. Pure data +
  // pure functions so test/signout.test.mjs can assert them without a DOM.
  HS.ACCOUNT_LOCAL_KEYS = ['myZip', 'activeProp', 'follows', 'topicPrefs', 'dismissed', 'myCommunities'];
  // Remove every account-scoped key from a Storage-like object (get/removeItem).
  // Keys are 'hs:'-prefixed on disk, matching the LS helper in shell.js.
  function clearAccountLocal(ls) {
    if (!ls || typeof ls.removeItem !== 'function') return;
    HS.ACCOUNT_LOCAL_KEYS.forEach(k => { try { ls.removeItem('hs:' + k); } catch (e) {} });
  }
  HS.clearAccountLocal = clearAccountLocal;
  // Pure core of HS.hasArea: has the visitor set their own area (a real active
  // property OR a saved ZIP)? shell.js's HS.hasArea delegates here so the "is this
  // a sample view" decision has one testable definition. After clearAccountLocal,
  // both inputs are null -> false -> HS.isSample() is true.
  HS.hasAreaFrom = function (activeProperty, myZip) { return !!(activeProperty || myZip); };
})();
