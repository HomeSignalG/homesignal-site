...
(function () {
  ...
  HS.nationalPlaneResult = function (r) {
    const fail = function (reason, http) {
      return { status: HS.NATIONAL_PLANE_UNAVAILABLE, reason: reason, http: http, records: [] };
    };
    ...
    return { status: HS.NATIONAL_PLANE_OK, reason: null, http: http, records: r.body };
  };
  ...
  const data = {
    ...
    async nationalDataCenters(zip, home, radiusMi) {
      ...
      if (isSeed()) {
        ...
        seeded.complete = true; seeded.plane = { status: 'ok', reason: null, http: null };
        return seeded;
      }
      ...
      out.complete = outcome.status === 'ok';
