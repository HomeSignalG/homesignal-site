...
(function () {
  ...
  const data = {
    ...
    async properties() {
      ...
      // Signed-out (or demo) visitors see the Del Valle SAMPLE homes so the dashboard /
      // properties pages preview instead of rendering empty — the same sample the other
      // pages show. FICTIONAL addresses, flagged sample:true so record pages refuse to
      // measure a made-up house and HS.realHome() treats them as non-home.
      if (!HS.state || !HS.state.session || HS.state.session.demo) {
