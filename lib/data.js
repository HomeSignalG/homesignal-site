...
(function () {
  ...
  const data = {
    ...
    // Phase-2 impact analyses (development_impact_analyses, public-read RLS) —
    // one row per official record document, keyed by source_ref. Returns a map
    // {source_ref -> row}; missing refs simply aren't in the map (cards fall
    // back to the conservative metadata path — never blocked on analysis).
    async impactAnalyses(sourceRefs) {
      if (isSeed()) return {};
      const refs = (sourceRefs || []).filter(Boolean);
      const map = {};
      for (let i = 0; i < refs.length; i += 100) {
        const { data } = await sb().from('development_impact_analyses')
          .select('*').in('source_ref', refs.slice(i, i + 100));
        (data || []).forEach(r => { map[r.source_ref] = r; });
      }
      return map;
    },
