...
(function () {
  ...
  const data = {
    ...
    async nationalDataCenters(zip, home, radiusMi) {
      zip = zip || CFG.DEFAULT_ZIP;
      home = homeFor(zip, home);
      if (isSeed()) return withDistance((window.HS_SEED.national_dc || []).slice(), home);
      const { data, error } = await sb().rpc('national_dc_for_zip',
        { p_zip: zip, p_radius_mi: radiusMi || 5 });
      if (error) return [];                       // a failed national read never breaks the page
      ...
      return withDistance(rows.map(normProject), home);
    },
