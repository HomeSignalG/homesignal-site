...
(function () {
  ...
  const data = {
    ...
    // ---- PROPERTY CARD reads (property-card.html) --------------------------------
    // The card is keyed by the ENGINE'S canonical address string, exactly as
    // homesignalmap.html?addr= already is (docs/property-reports-cache.sql). Both reads
    // below return null rather than an empty-but-plausible object, because the card has to
    // be able to say "no property record is cached for this address" as a distinct answer
    // from "we checked and there is nothing at it".
    async propertyReport(addr) {
      if (!addr) return null;
      const key = String(addr).trim();
      if (isSeed()) {
        const seeded = (window.HS_SEED && window.HS_SEED.property_reports) || {};
        const hit = Object.keys(seeded).find(k => k.toLowerCase() === key.toLowerCase());
        return hit ? seeded[hit] : null;
      }
      const { data } = await sb().from('property_reports')
        .select('address,zip,county,state,lat,lng,counts,sites,sources_checked,paywall,source_vintage,refreshed_at')
        .eq('address', key).limit(1);
      return (data && data[0]) || null;
    },
    // The ENTITY TRACK RECORD payload for one address: who the companies are, how they relate to
    // the property and to each other, and every enforcement record attributed to each of them.
    // Store + read function: docs/property-card-entity-track-record.sql.
    //
    // THE RETURN IS A STATUS, NOT A PAYLOAD-OR-NULL, because the three outcomes are three
    // different facts and the card renders each differently:
    //   ok      — we read the store; whatever it holds is the answer
    //   absent  — the store is not installed for this deployment, so nothing has been checked
    //   error   — we tried to read and failed, so we do NOT know what it holds
    // Collapsing `error` into `absent` would render a failed read as "not checked yet", which is
    // the same class of defect as rendering it as zero (PR #662).
    async entityTrackRecord(addr) {
      const empty = { entities: [], enforcement_records: [] };
      if (!addr) return { status: 'absent', ...empty };
      const key = String(addr).trim();
      if (isSeed()) {
        const seeded = (window.HS_SEED && window.HS_SEED.entity_track_record) || {};
        const hit = Object.keys(seeded).find(k => k.toLowerCase() === key.toLowerCase());
        return hit ? { status: 'ok', ...empty, ...seeded[hit] } : { status: 'absent', ...empty };
      }
      try {
        const { data, error } = await sb().rpc('property_card_entity_track', { p_address: key });
        if (error) {
          // PostgREST reports an uninstalled function as PGRST202; Postgres as 42883. Either way
          // the store does not exist here, which is "nothing checked", not "the read failed".
          const missing = error.code === 'PGRST202' || error.code === '42883'
            || /does not exist|could not find/i.test(error.message || '');
          return { status: missing ? 'absent' : 'error', ...empty };
        }
        return { status: 'ok', entities: (data && data.entities) || [],
          enforcement_records: (data && data.enforcement_records) || [] };
      } catch (e) {
        console.warn('[data] entityTrackRecord failed', e);
        return { status: 'error', ...empty };
      }
    },
    // Resolve the canonical address for ONE map record. app_projects carries no address
    // column, so the address lives only on the engine's own cached site objects — the same
    // ZIP row homesignalmap.html reads to draw its map. Matched on the record URL, which is
    // mandatory on every emitted site (the anti-fabrication gate), never on a name.
    // Returns null when nothing matches: an unresolved parcel is a real answer.
    async canonicalAddrFor(zip, rec) {
      if (!zip || !rec) return null;
      const direct = (window.HS && HS.card && HS.card.keyOf) ? HS.card.keyOf(rec) : null;
      if (direct) return direct;
      const url = rec.source_ref || rec.record_url || null;
      if (!url) return null;
      if (isSeed()) {
        const sites = (window.HS_SEED && window.HS_SEED.property_sites) || [];
        const s = sites.find(x => x.record_url === url);
        return (s && (s.canonical_addr || s.location_addr)) || null;
      }
      const { data } = await sb().from('development_reports').select('sites').eq('zip', zip).limit(1);
      const sites = (data && data[0] && data[0].sites) || [];
      for (let i = 0; i < sites.length; i++) {
        const s = sites[i];
        if ((s.record_url || s.url) === url) return s.canonical_addr || s.location_addr || null;
      }
      return null;
    },
