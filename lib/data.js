...
(function () {
  ...
  // How far back the recent-meetings fallback looks when a ZIP has nothing upcoming.
  // 30 days is measured, not chosen: every one of the 103 Utah ZIP pages with an empty
  // tile had a meeting inside this window, and widening it buys nothing while ageing
  // what the page calls "recent".
  const RECENT_MEETING_DAYS = 30;
  HS.RECENT_MEETING_DAYS = RECENT_MEETING_DAYS;
  ...
  // THE READ'S VERDICT RIDES WITH THE ROWS THROUGH A RE-DISTANCE, because `.map()` returns a
  // fresh array and would otherwise LAUNDER a refused read into an ordinary empty list. That
  // is not hypothetical: property.html re-runs `HS.withDistance(projects, p)` before sorting,
  // so every caller downstream of that line saw a bare `[]` with no way left to tell "nobody
  // has measured this ZIP" from "measured, and empty" — the exact conflation these fields
  // exist to prevent. Carrying them is a no-op for a seed array (which carries none) and for
  // any caller that does not read them.
  function withDistance(items, home) {
    ...
    // Copied only when the source actually carried one, so an array that never had a verdict
    // does not acquire a fabricated `complete:true` on its way through.
    if (items && typeof items === 'object') {
      if ('complete' in items) out.complete = items.complete;
      if ('outcome' in items) out.outcome = items.outcome;
      if ('reason' in items) out.reason = items.reason;
    }
    return out;
  }
  ...
  // THREE OUTCOMES, NOT TWO, because `complete:false` alone cannot tell a resident-facing
  // caller WHY. The development producer answers a ZIP whose whole-ZIP geography is not
  // established with a deliberate envelope — {unavailable:true, zip_geography_status} — and
  // that is a DIFFERENT fact from a transport failure. Measured 2026-09-22: 709 of 12,722
  // canonical ZIPs (706 not_measured, 3 unknown) take that branch, and the ZIP page rendered
  // every one of them as "No permit or planning records on file for this ZIP yet" — a
  // verified-absence claim over, in 84011's case, 60 source-stated development rows.
  //
  // `outcome` IS NOT DECIDED HERE. lib/zip-authoritative.js owns the status vocabulary and
  // its interpretation for both producers of this geography truth; this function owns
  // TRANSPORT completeness and delegates the semantics. A second classifier here would be
  // the rival definition that module's header forbids.
  async function rpcAllRows(zip, kind) {
    for (let attempt = 0; attempt < 2; attempt++) {          // one retry, as fetchAllPages does
      ...
      if (!error && Array.isArray(data)) {
        return { rows: data, complete: true, outcome: 'complete', reason: null };
      }
      // A DELIBERATE, DETERMINISTIC UNAVAILABLE ENVELOPE — never retried. The producer read
      // geo.maps_zip_geography_status and said so; asking again puts the identical question
      // to the identical state and gets the identical answer, so the retry is pure waste on
      // every one of those 709 pages, on every load. A transport error falls through and
      // KEEPS the retry, which is the case the retry was written for.
      if (!error && data && typeof data === 'object' && data.unavailable === true) {
        return {
          rows: [], complete: false,
          outcome: (HS.zipAuthOutcome ? HS.zipAuthOutcome(data) : 'unavailable'),
          reason: data.zip_geography_status || null
        };
      }
    }
    return { rows: [], complete: false, outcome: 'unavailable', reason: null };
  }
  ...
  // ── THE ONE MAP 1 DATA-CENTRE CONTRACT (docs/map1-dc-publication.sql) ────────────────────
  // Every data-centre record Map 1 draws on a ZIP page comes from ONE server read,
  // public.map1_dc_zip_members(p_zip): canonical entities (Compute Atlas, Epoch AI and any
  // source onboarded to the canonical plane) plus the transitional legacy-OSM compatibility
  // population, each admitted by the canonical point-in-ZIP predicate. No caller asks which
  // source a record came from or unions sources itself. The pin status is the server's
  // `map_status` (ONE lifecycle map, server-side) — a proposed project is never drawn as
  // Approved. There is no radius and no distance: membership is the row's own verdict.
  HS.MAP1_DC_RPC = 'map1_dc_zip_members';
  HS.map1DcSite = function (r) {
    return {
      id: r.source_key, name: r.project_name,
      status: r.map_status,
      normalized_status: r.normalized_status, raw_status: r.raw_status,
      type: 'datacenter', record_kind: 'national_project',
      ...
      record_url: r.source_url, url: r.source_url,   // the anti-fabrication gate reads this
      source_name: r.source_name, source_licence: r.source_licence || null,
      source_key: r.source_key,
      canonical_entity_id: r.canonical_entity_id || null,
      publication_basis: r.publication_basis || null,
      quality_flags: r.quality_flags || [],
      // the server's canonical verdict for THIS ZIP — only 'member' is ever admitted
      zip_membership: r.zip_membership,
      relevance: 'development'
    };
  };
  // Attribution is a licence obligation (CC BY 4.0, ODbL), derived ONLY from sites that can
  // reach the page and carrying each source's OWN licence, never one hard-coded string.
  HS.map1DcCredits = function (sites) {
    const seen = {};
    (sites || []).forEach(function (s) {
      if (s && s.source_name && s.record_url) {
        seen[s.source_name + (s.source_licence ? ' (' + s.source_licence + ')' : '')] = 1;
      }
    });
  ...
  const data = {
    ...
    async projects(zip, home) {
      ...
      // facilities are NOT development — they come from facilities(), never here. (Since
      // 2026-09-25 the ZIP page renders no facility list in What's Changing; Map 1's
      // Regulatory Records overlay is where they are shown.)
      zip = zip || CFG.DEFAULT_ZIP;
      ...
      // The geography outcome rides WITH the rows, because `complete:false` alone cannot
      // tell the page whether this ZIP is unmeasured or the read simply failed — and
      // lib/community-page.js must not re-derive either from `out.length` (rule 1).
      out.outcome = res.outcome;
      out.reason = res.reason;
      return out;
    },
    async facilities(zip, home) {
      // Regulated facilities (EPA FRS/ECHO; lifecycle unknown — status 'On file', since FRS
      // states no lifecycle and registration is not operation) — environmental context,
      ...
      const out = withDistance(res.rows.map(normProject), home);
      out.complete = res.complete;
      // Carried for symmetry only. The facility branch never reaches the geography gate —
      // it is served by the producer's LEGACY branch, which always coalesces to '[]' — so
      // this is 'complete' on every ZIP, measured 310/310 in Utah including all 18.
      out.outcome = res.outcome;
      out.reason = res.reason;
      return out;
    },
    // NATIONAL DATA-CENTER PLANE — the third plane, and the ONLY one that is not
    // jurisdiction-gated. It reads THE ONE CONTRACT (HS.MAP1_DC_RPC above), which asks nothing
    // about registry coverage, so a record plots in a county with no permit/planning connector.
    ...
    // ⚠️ LICENCES (CC BY 4.0, ODbL): any surface that displays these records must carry the
    // attribution exposed as `source_name` + `source_licence` on every row (HS.map1DcCredits).
    async nationalDataCenters(zip, home) {
      zip = zip || CFG.DEFAULT_ZIP;
      home = homeFor(zip, home);
      if (isSeed()) {
      ...
      // This used to call national_dc_for_zip with a 5-mile radius around the ZIP centroid —
      // proximity, not membership. It now reads the one contract; membership is per row.
      const { data, error } = await sb().rpc(HS.MAP1_DC_RPC, { p_zip: zip });
      ...
      // One mapping (HS.map1DcSite), shared with homesignalmap.html; only a 'member' verdict
      // for THIS ZIP is admitted.
      const rows = outcome.records.map(HS.map1DcSite).filter(function (s) {
        return s.zip_membership === 'member';
      });
    ...
    async meetings(zip, home) {
      ...
      const scopeToPlace = rows => (rows || []).filter(m => {
      ...
      const { data } = await sb().from('meetings').select('*').in('community_id', ids)
        .gte('meeting_date', cutoff).order('meeting_date', { ascending: true }).limit(24);
      const scoped = scopeToPlace(data);
      if (scoped.length) {
        const out = withDistance(scoped.map(normMeeting), home);
        out.recent = false;
        return out;
      }

      // ---- RECENT FALLBACK: the tile is empty because of HOW a county PUBLISHES ----
      // Measured 2026-09-21. 103 of Utah's 310 ZIP pages showed an empty Upcoming
      // Meetings tile, across 13 counties, and NOT ONE was a wiring defect: all 13
      // ingested that same day and ~72% of their notices arrive before the meeting.
      // The cause is publishing CADENCE. Salt Lake County posts its whole year of
      // council meetings at once (lead times 240-338 days, newest 2027-06-08) so its
      // board is never empty; Weber County posts one notice 1-2 days ahead, so the
      // instant a meeting starts the tile goes blank until the next notice appears —
      // while filing MORE notices than Davis (19 vs 16 in 30 days). Feed count predicts
      // nothing: Weber has 14 meeting feeds and 0 upcoming, Salt Lake 13 and 38.
      //
      // So the content exists and the page was refusing to show it. Of those 103 ZIPs,
      // 103 had a meeting within the last 30 days — ZERO had nothing recent.
      // Nationally this fills 1,571 more canonical ZIP pages (5,089 -> 6,660).
      //
      // ⚠️ THE CALLER MUST RELABEL. A meeting last Tuesday is NOT upcoming, and filling
      // the tile by making a false claim about a public body would be worse than the
      // empty tile. `out.recent` says which set this is; lib/community-page.js switches
      // every "upcoming" label on it, and test/meetings-recent-fallback.test.mjs fails
      // if a caller renders this set under the upcoming wording.
      const since = new Date(Date.parse(cutoff) - RECENT_MEETING_DAYS * 86400000).toISOString();
      const { data: past } = await sb().from('meetings').select('*').in('community_id', ids)
        .gte('meeting_date', since).lt('meeting_date', cutoff)
        .order('meeting_date', { ascending: false }).limit(24);
      const recent = withDistance(scopeToPlace(past).map(normMeeting), home)
        .map(m => Object.assign({}, m, { recent: true }));
    ...
    // The ZIP's OWN community row (level 'zip') — where the "What is changing in my
    // zip code?" email selection is filed. resolveCommunity already ranks the ZIP level
    // first; a city or county that merely CONTAINS the ZIP is not the ZIP, so it returns
    // null and the caller refuses, rather than filing a selection the database refuses
    // (user_subscriptions_maps_zip_scoped) or that no MAPS post could ever match.
    async zipCommunity(zip) {
      zip = String(zip || '').trim();
      if (isSeed() || !/^\d{5}$/.test(zip)) return null;
      const node = await resolveCommunity(zip);
      return node && node.level === 'zip' ? { id: node.id, name: node.name || '' } : null;
    },
  ...
  // ---- "What is changing in my zip code?" — Map 1's email sign-up (founder, 2026-09-25) ----
  // Email copies of the Bluesky MAPS posts about ONE ZIP. It is a STREAM in the one
  // subscription store (docs/alert-subscription-canonical-a12.sql), never a second store:
  //   * the writer is enable_area_email_alerts, the one ADDITIVE consent writer;
  //   * the selection is filed on the ZIP's OWN community row, because a MAPS post is
  //     about one ZIP (social_posts.zip) — a county anchor could never be matched to a
  //     post, and the database refuses it (user_subscriptions_maps_zip_scoped);
  //   * p_marketing_consent is FALSE: this tap asks for the ZIP's posts, not marketing
  //     (founder contract F). It never revokes marketing consent given elsewhere.
  // The topic is the founder's wording, word for word, and the one catalog row a12 adds.
  HS.MAPS_EMAIL_STREAM = 'maps';
  HS.MAPS_EMAIL_TOPIC = 'What is changing in my zip code?';
  HS.mapsOptinRpcArgs = function (email, zipCommunityId, zip, consentVersion, consentCopy, referral) {
    const ref = referral || {};
    const topics = {};
    topics[HS.MAPS_EMAIL_STREAM] = [HS.MAPS_EMAIL_TOPIC];
    return {
      p_email: email, p_community_id: zipCommunityId, p_zip_code: zip,
      p_topics: topics, p_consent_version: consentVersion,
      // The canonical writer's parameter name; with p_marketing_consent=false the
      // server records this string as the ALERT consent copy only (audit trail).
      p_marketing_consent_copy: consentCopy,
      p_marketing_consent: false,
      p_referral_source: ref.source || null,
      p_referral_campaign: ref.campaign || null
    };
  };
})();
